...
# Auto-generates an Open WebUI -> planner API key if one doesn't exist.
...
# -----------------------------------------------------------------------
# Ensure an Open WebUI -> planner API key exists in the cluster secret.
# If the secret doesn't exist or still has the placeholder value,
# generate a real key and create/update the secret.
# -----------------------------------------------------------------------
ensure_webui_planner_key() {
    local ns="synesis-webui"
    local secret_name="webui-api-key"
    local existing_key=""
    ...
    if [[ -z "$existing_key" ]] || [[ "$existing_key" == "sk-synesis-change-me" ]]; then
        WEBUI_API_KEY="sk-synesis-$(openssl rand -hex 24)"
        log "Generating Open WebUI planner API key..."
        oc create secret generic "$secret_name" \
            -n "$ns" \
            --from-literal=api-key="$WEBUI_API_KEY" \
            --dry-run=client -o yaml | oc apply -f -
        log "  Key stored in secret $ns/$secret_name"
    else
        WEBUI_API_KEY="$existing_key"
        log "Open WebUI planner API key already exists in $ns/$secret_name"
    fi
}
...
# started, the pod never receives OPENROUTER_API_KEY until recreated.
reconcile_provider_api_keys() {
    ...
    if [[ -z "$key" ]] || [[ "$key" == "sk-or-v1-REPLACE_ME" ]]; then
        log ""
        log "OPENROUTER_API_KEY missing in $ns/$secret_name — direct provider calls will fail."
        log "  Re-running provider key setup..."
        ensure_openrouter_key

        oc rollout restart deployment/synesis-planner-ts -n synesis-planner 2>/dev/null || true
        oc rollout restart deployment/synesis-yarn -n synesis-yarn 2>/dev/null || true
        return 0
    fi

    # Valid key in cluster. Admin provider-key APIs restart direct consumers after changes.
}

# Heal webui-api-key if an older release applied the placeholder value.
reconcile_webui_client_secret() {
    local wu="synesis-webui"
    local wk="" changed="false"
    ...
    if [[ -z "$wk" ]] || [[ "$wk" == "sk-synesis-change-me" ]]; then
        local newk
        ...
        oc create secret generic webui-api-key \
            ...
            --dry-run=client -o yaml | oc apply -f -
        log "  Generated new Open WebUI planner client key"
        changed="true"
    fi
...
ensure_openfga() {
    ...
    # Create openfga database idempotently.
    local pod=""
...
[[ "$MODE" == "api" ]] && log "LLM route mode: direct provider routes from Admin registry"
log ""
...
log ""
log "Reconciling Open WebUI client secret (post-apply)..."
reconcile_webui_client_secret
...
if [[ "$MODE" == "api" ]]; then
    ...
    log "  Planner calls OpenAI-compatible providers directly using Admin Model Registry routes"
    log "  Configure provider endpoints and keys in Admin UI > Model Registry"
else
...
ROUTE_HOST=$(oc get route synesis-api -n synesis-planner -o jsonpath='{.spec.host}' 2>/dev/null || echo "not-yet-created")
...
log "  API auth:      Personal Access Token from Admin UI"
...
log ""
log "Quick test:"
log "  curl -s https://$ROUTE_HOST/v1/models -H 'Authorization: Bearer <your-personal-token>' | python3 -m json.tool"
