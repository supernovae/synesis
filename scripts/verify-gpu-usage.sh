...
for deploy in synesis-router synesis-critic synesis-general synesis-coder; do
    ...
    for c in vllm-router vllm-critic vllm-general vllm-coder kserve-container ""; do
        if [[ -z "$c" ]]; then
            oc exec -n "$NAMESPACE" "$pod" -- nvidia-smi --query-gpu=name,memory.used,memory.total --format=csv,noheader 2>/dev/null && gpu_found=true && break
        else
            oc exec -n "$NAMESPACE" "$pod" -c "$c" -- nvidia-smi --query-gpu=name,memory.used,memory.total --format=csv,noheader 2>/dev/null && gpu_found=true && break
        fi
    done
    if [[ "$gpu_found" == "true" ]]; then
        log "$deploy: GPU memory usage ✓"
    else
