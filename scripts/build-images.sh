...
# Base images are built first so consumer images can reference them.
#
# Usage:
...
#
# Base images MUST appear before any consumer that depends on them.
# base-api must precede base-ml (base-ml FROM base-api).
...
if [[ "$BASES_ONLY" == "true" ]]; then
    ...
    IMAGES=("${BASE_IMAGES[@]}" "${SERVICE_IMAGES[@]}")
fi
