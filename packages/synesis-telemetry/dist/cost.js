...
export function computeCost(usage, rates, cachedMultiplier = 0.1) {
    const uncached = Math.max(0, usage.prompt_tokens - usage.cached_prompt_tokens);
    const cachedRate = rates.cached_input_per_million ?? rates.input_per_million * cachedMultiplier;
    const estimated = (uncached / 1e6) * rates.input_per_million +
...
export function resolveEffectiveCost(estimated, actual) {
