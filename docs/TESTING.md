...
|----------|----------------|-----------------|
| [`.github/workflows/lint.yml`](../.github/workflows/lint.yml) | ShellCheck; **Ruff** (`base/`); yamllint; kustomize build (overlay matrix); Hadolint; **admin** pytest; **yarn-ts** `tsc` + Vitest; **mcp-ts** + **admin-mcp-ts** Vitest; **planner-ts** Vitest; **synesis-context-trust** package tests | Yes (required jobs) |
...
### 2.2 Manual / optional live HTTP (deployed planner-ts)

Use `scripts/validate-intent-live.py` (defaults to `http://localhost:8080`) after `oc port-forward svc/synesis-planner-ts 8080:8080 -n synesis-planner`. See [DEVELOPMENT_CHECKS.md](./DEVELOPMENT_CHECKS.md).

### 2.3 OpenAI-compat probe (manual / optional workflow)

```bash
export SYNESIS_PROBE_PLANNER_URL=http://127.0.0.1:8080
...
- **Command:** `cd base/yarn-ts && npm ci && npx vitest run` (see `lint.yml` `yarn-ts-tests` for root `npm ci` + workspace build ordering).
- **CI:** `yarn-ts-tests` job in `lint.yml`.
...
| `model` id aliases | Yes — `Synesis`, `Synesis Thinking`, `synesis-agent`, etc. | Yes — `synesis-yarn` | Normalization tests in planner-ts |
...
|------------|--------|-------------------|
| Chat + streaming + usage (above) | Implemented | Extend planner-ts Vitest / live tests when behavior changes |
...
1. **Planner-ts:** Optional **`openai` Python client** smoke against localhost (dev-only) for wire-format parity with external SDKs.
2. **Yarn-ts:** Expand contract matrix for **tool_calls** round-trip and **stream** final chunk shape (align with OpenAI client expectations).
3. **Explicit 501 (optional):** For `POST /v1/images/generations` etc., return structured JSON `{"error": {"message": "Not implemented", "type": "not_implemented"}}` if we want friendlier SDK errors than 404.
