...
**Related:** [LORA_TRAINING_GUIDE.md](LORA_TRAINING_GUIDE.md), [WORKFLOW_PLANNER.MD](chat/WORKFLOW_PLANNER.MD)
...
| 2026-03-07c | Knowledge path depth fix: depth-encouraging pinned context, prescriptive planner, per-section depth rules, temp 0.3, improved web search queries, curated RAG docs | 6.5-7/10 | Major jump. Better role separation, hybrid control patterns, more buildable. Still lacks calibration structure, decision policy, and concrete failure modes. |
| 2026-03-07d | Parallel per-section generation, provenance/authority RAG, web search trust classification | 6.5-7/10 | Per-section generation likely did NOT activate for benchmark — software_architecture domain detection required 2 keyword hits but prompt only matched 1 ("architecture"). Output was still monolithic. |
| 2026-03-07e | Fix domain keywords, calibration enforcement in taxonomy/executor/critic, anti-boilerplate steering | ~7/10 (structure 8, specificity 7, prompt compliance 5) | Per-section generation activated (process diagrams visible). Private/public split, deterministic tooling, practical roadmap. Still missing calibration structure, decision policy, concrete failure modes, retrieval realism. |
| 2026-03-07f | Planner guarantees dedicated steps for decision policy, calibration, and failure modes; writer preserves calibration labels; token budget increase (section 3072, writer 12288); narrative depth steering | TBD | Addressing prompt compliance gap. |
| 2026-03-07g | **Always-plan architecture**: all non-trivial knowledge tasks flow through planner → writer → critic. Continuous difficulty (0-1) scales section count, token budgets, web search budget, critic strictness. CRAG-style corrective retrieval detection. Router handles all routing. | TBD | Major architectural simplification. Depth is a dial, not a gate. |
| 2026-03-07h | **Toulmin argumentation rubric**: critic evaluates claim/grounds/warrant/rebuttal structure instead of domain-specific checklists. Writer and planner instruct Toulmin structure. 3+ uncommitted "X or Y" decisions = BLOCKING. Domain-agnostic quality enforcement. | TBD | Generalizable commitment enforcement across all domains. |
...
1. **Deliverable merging.** When a user prompt lists 8 explicit deliverables, the 8B planner merges them into 4-5 generic steps. It fails to preserve 1:1 mapping between user requests and plan steps. This directly causes the executor to skip sections the user asked for.
...
3. **Section title quality.** Plan step `action` fields tend toward generic labels ("Define goals and constraints") rather than preserving the user's specific wording ("State the main design goals"). This degrades the executor's ability to map sections to the user's intent.
...
## General / Executor (Qwen3-32B FP8)
...
- **LoRA candidate:** Executor LoRA is Priority 2. Training on (architecture prompt + planner outline + constraints → response with concrete choices, fact/assumption/recommendation separation, timeline-scoped recommendations) would address the remaining gaps.
...
1. **Domain keyword threshold too strict.** `software_architecture` required `min_hits: 2` from a narrow keyword list. The benchmark prompt said "architecture" (1 hit) but used "design" which wasn't in the list. This caused the domain to not activate, bypassing the entire deep-dive pipeline, `_DEEP_DIVE_SUFFIX` instructions, and taxonomy-specific persona/instructions.
