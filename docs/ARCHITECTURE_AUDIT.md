...
| **Prompt suite in CI** | **RESOLVED.** `prompt-regression.yml` runs router governance + prompt suite (adversarial, retrieval_quality, node_routing, grounding) on planner changes. Admin Testing Labs for customer replay/comparison. | See `docs/TESTING.md` §9 for full workflow reference. |
| **Summarizer** | **RESOLVED.** `summarizer_model_url` defaults to the `synesis-summarizer` InferenceService endpoint (Qwen2.5-0.5B CPU for self-hosted). Production: `deployment.yaml` overrides to LiteLLM proxy; admin registry / Helm static values route `synesis-summarizer` to provider (grok-4-fast via xAI). `litellm-config.yaml` static fallback routes to router pod. `pivot_summary_enabled=True` — no longer stubbed. | Model routing documented in `config.py`, `history_summarizer.py`, and this table. |
...
| **Faithfulness / grounding automation** | **IN PROGRESS.** Five runtime layers enforce grounding: plan-gate phantom URL/citation detection, router `publish_knowledge_gap` for low-confidence packets, critic deterministic URL whitelist diff (`hallucinated_citation`), citation-rate heuristic (`evidence_underuse` at difficulty >= 0.6), and final-scrubber source pruning. Both modes now in `FAILURE_MODE_TAXONOMY`. `knowledge_backlog_enabled` flag respected. Pytest covers URL diff, citation rate, zero-evidence leniency, taxonomy membership, citation preservation. `grounding` category (5 prompts + `reject_terms`) in prompt suite and `prompt-regression.yml` CI. | Activate live validation ring (`SYNESIS_VALIDATION_ENABLED=true`) with populated RAG; automate Testing Labs citation-score baseline comparison. |
