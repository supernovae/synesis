...
2. **MCP-style filters** — combine `language`, `artifact_kind`, `content_type`, `index_decision` via planner-ts retrieval filters (see `base/planner-ts/src/retrieval/metadata-filter.ts` and `base/planner-ts/src/retrieval/rag-client.ts`).
