...
- **Post-apply** `patch_planner_retrieval_and_web` (default **on**): sets TEI, NornicDB, SearXNG URL, and web enabled on `synesis-planner-ts` so values persist across applies. **Off:** `SYNESIS_DEPLOY_PLANNER_RETRIEVAL=false`.
