...
2. **Maximize prefix cache hits** -- Static content first in supervisor/critic prompts; vLLM caches Tier 1/2 KV states.
...
| Item | Description |
|------|-------------|
| **Prefix caching** | Supervisor-critic runtime with `--enable-prefix-caching` for Qwen3-8B FP8-dynamic. Executor uses FP8 KV cache instead (mutually exclusive with prefix caching). |
...
| Feature | Where | Notes |
|---------|-------|------|
| **Prefix caching** | Supervisor-Critic | `--enable-prefix-caching`. Incompatible with FP8 KV cache. |
...
| **Guided JSON decoding** | Supervisor, Critic | `with_structured_output` via vLLM JSON schema. |
