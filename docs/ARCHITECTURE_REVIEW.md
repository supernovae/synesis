...
| 3. Test whether the artifact deserves to exist | Real public and private workflows against native source tools and an established knowledge alternative. Record setup/build cost, needed capability and user corrections. Run bounded model ablations only for quality questions that remain. | An exhaustive contest among every model, gateway and database. |
