...
**Direction selected: knowledge-first, local by default; hosted retrieval optional.** This is an economic and maintenance decision based on the code, current alternatives and the maintainer's requirements. It is not a measured claim that this design improves model task success. The local source-pack CLI and MCP reader are implemented as of September 10, 2026. The control plane and deployment stack are removed. Product-fit evidence and decisions about remaining optional preparation handlers are outstanding; hosted retrieval remains conditional on an actual need. The local package is usable from a checkout; no registry release is claimed.
...
The [inventory](../evals/architecture/inventory.json) records the baseline commit and methodology. At that baseline, default Compose contained six services; Helm defaults enabled sixteen workloads and eighteen replicas with 23.3125 GiB of declared memory requests. Those deployment files are now removed. This excludes other infrastructure and inference. Requests are configuration, not measured consumption or a cloud bill. The code establishes deployment complexity; the maintainer establishes that its operating cost was unacceptable. It does not identify which individual service caused the bill.
...
| Responses streaming is buffered compatibility output. | [Responses route](https://github.com/supernovae/synesis/blob/efd751515f362864697dec6897981a169894048e/base/yarn-ts/src/routes/responses-routes.ts) invokes Chat with streaming disabled, then synthesizes events. | Retire that translation from the target. Direct native endpoints preserve their own semantics. |
| Planner normalization loses non-text content. | [API schemas](https://github.com/supernovae/synesis/blob/efd751515f362864697dec6897981a169894048e/base/planner-ts/src/api-schemas.ts), messageContentToText. | Do not put this path in front of modern multimodal clients. Accepting a schema is not implementing its capability. |
...
| Timeout responses do not necessarily stop work. | [Graph runner](https://github.com/supernovae/synesis/blob/efd751515f362864697dec6897981a169894048e/base/planner-ts/src/graph.ts) races a timeout against node execution. | Retained jobs need cancellation and publication ownership; a timeout flag is not cancellation. |
...
| Retrieval evaluation rewards enrichment mechanically. | [Evaluator](https://github.com/supernovae/synesis/blob/efd751515f362864697dec6897981a169894048e/base/admin/app/services/rag_eval_harness.py) awarded points for cards, producer-reported quality and unspecified checks. | **Corrected before retirement:** enrichment became diagnostic only, with scoring limited to configured expectations. The evaluator and its tests are now removed with Admin; these results are historical. |
| Knowledge packaging assumes derived graph/vector materialization. | [SynPack reference](https://github.com/supernovae/synesis/blob/efd751515f362864697dec6897981a169894048e/docs/SYNPACKS.md), indexer and NornicDB retrieval. | Make source identity the portable contract; rebuild search indexes locally. Generated records cannot become authoritative evidence. |
| Repository guidance encodes obsolete topology. | Constitution, graph-state and ML-boundary rules under .cursor/rules. | **Updated:** service-specific rules are removed. Current guidance covers local source integrity, client boundaries and the reduced toolchain. |

These are findings from the pre-cutover code. Links to removed implementations resolve to the last pre-removal commit. This is not a complete security assessment or a claim that every historical route was reviewed.
...
The first usable slice is [one TypeScript CLI/library](../packages/synesis-mcp/) for pack validation, indexing, source reading and MCP registration. It replaces the Planner-backed MCP entry point and depends only on the MCP SDK, Zod and built-in Node modules. The [source-pack guide](SOURCE_PACKS.md) specifies the format, explicit-root rules, immutable identities and local access boundary. Reuse Python extraction handlers as bounded optional build tools where they save work; do not require the existing indexer image or Python ML stack merely to read a pack. If HTTP hosting is ever justified, reuse the same query functions and add an explicit remote authorization boundary. A second implementation in another language, transport-specific policy copies and an extensible database framework would recreate the maintenance problem.

The source archive contains a manifest, source revision/content hashes, explicit version, source paths and line-addressable text, attribution and redistribution metadata. Saved HTML records the original file identity in derived Markdown; its citations address Markdown lines rather than original HTML positions. The SQLite index is rebuildable derived data, not the only copy of the source or a required uploaded database. Core operations are pack discovery/version resolution, search, bounded source read, validation and explicit build/import/delete. Pagination and retrieval budgets should be explicit. Version selection must be visible; “latest” is mutable and must resolve to a recorded snapshot.
...
The local MCP entry point is implemented. Planner/Yarn/Admin, their hosted MCP services, assistant loop, model/harness adapters and exclusive shared packages are deleted. Their deployment recipes, image builds, live evaluations and operator clients are also removed. The disposition below distinguishes functionality preserved in the reader from capabilities delegated or deliberately retired; it does not imply feature parity with the former platform.
...
- Historical corrections to the former Admin evaluator removed enrichment bonuses before its retirement. Its implementation and the paired-result analysis runtime are now removed; neither is a current retrieval-quality gate.
...
| 5. Complete the fix-forward cutover in the repository | **Control-plane cutover implemented:** Planner/Yarn/Admin, hosted MCP, exclusive shared packages, deployment/config/docs/CI, model-serving and identity services, old indexer orchestration and architecture-lab runtime are deleted. The README describes the local artifact. Optional source handlers remain candidates for bounded commands or deletion; no migration or replacement platform is introduced. | Migrations, aliases, dual-write modes or indefinite maintenance of the architecture lab. |

The artifact trial in step 3 is a deliberate stop point before building another platform. If existing tools satisfy the needed outcomes, retire the remaining artifact rather than build step 4 as a replacement service. If only pack production/distribution is useful, publish a builder/reader and stop there. If shared retrieval earns its footprint, retain that narrow service. A universal model gateway or planner does not return merely because useful code remains in the repository.

The local artifact and optional saved-HTML preparation command are implemented. Source preparation now retains only handlers/text helpers and relevant tests. The command records original-file identity alongside derived Markdown. Other handlers are not exposed by the CLI or MCP server, and do not yet provide the same local-root/publication contract. Their presence does not promise connector support.

## Control-plane removal — September 10, 2026

The cutover removes Planner, Yarn, Admin/frontend, hosted MCP services, exclusive shared TypeScript packages, model/harness adapters, model-serving and identity services, sandboxes, gateway, databases, monitoring, Compose, Helm/Kustomize, infrastructure recipes, operator clients and image/live-evaluation workflows. Native clients own model requests, reasoning, tools, project roots, execution and approvals. There are no redirect aliases, migrations or configuration compatibility layers.

Ingestion no longer has Admin queue workers, staged S3 jobs, embeddings, inference enrichment, NornicDB writers, graph/vector pack builders or a service CLI. The retained handlers use standard Python logging; the telemetry package and its dependency chain are removed. The sole source-pack implementation remains the Node core. Saved HTML stays usable independently; remaining URL/PDF/code handlers require an actual preparation need and a bounded interface before becoming product features.

The cleanup also removes the runtime architecture lab and platform-specific guides/rules. Recorded inventories, smoke observations and the public Git/filesystem comparison remain identified historical evidence. Current CI checks the reader, optional preparation code, dependency pins, source analysis and documentation. Container/IaC checks disappeared with their targets; security checks for retained code and dependencies remain.

No running system was contacted or provisioned. Removing declared workloads and dependencies reduces repository maintenance surface; it is not a measured dollar saving, retrieval-quality gain or proof of equivalent model-service capabilities. The next product decision remains a real user workflow trial. Add another preparation command only for a demonstrated gap, and add hosting only if shared access earns its cost.

Cutover validation: the staged snapshot installs and builds independently, with 17 reader tests and 137 preparation tests passing. The lockfile regression test and a check using CI's pinned uv version also pass. A separately installed package tarball builds/imports/searches the example pack; source navigation finds the expected evidence across ten selected files. Lint, documentation links, Bandit's configured gate and npm/Python dependency audits pass. These are local code and artifact checks, not model or deployment measurements.
