...
patch_integrity_gate (no LLM) → [sandbox | lsp_analyzer | context_curator]
...
**Output:** `task_size`, `target_language`, `worker_persona`, `worker_prompt_tier`, `plan_required`, `bypass_supervisor`, `task_description`, etc.
...
**Role:** Router only. No architecture reasoning. EntryClassifier sets policy; Supervisor routes.
...
**Role:** Lint, security scan, scope validation. No LLM.
