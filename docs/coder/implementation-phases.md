...
- **Admin-Configurable Enrichment Model**: added `indexer-enrich` to `KNOWN_ROLES` in provider catalog; `SYNESIS_INDEXER_ENRICHMENT_MODEL` and `SYNESIS_INDEXER_ENRICHMENT_TIMEOUT` env vars replace hardcoded writer defaults; documented Groq/vLLM/OpenAI configuration with cost estimates in `docs/INDEXERS.md`
