...
### Harness Trust Scorecards

Governor regression and canary lanes produce release scorecards for coder
runtime trust. The scorecard combines scenario quality, governor behavior, and
tool-use discipline into one red-line summary.

Tracked scorecard KPIs:

| KPI | Why it matters |
|-----|----------------|
| `pass_rate` and `avg_score` | Overall scenario quality and completion |
| `intervention_rate` | Whether governor rules are firing more often than expected |
| `repeated_command_anomaly_rate` | Looping or repeated no-progress behavior |
| `avg_turns_to_resolution` | Efficiency and bounded follow-through |
| `read_edit_ratio` | Research/read discipline before edits |
| `whole_write_ratio` | Broad rewrite risk versus targeted patching |
| `premature_stop_signal_rate` | False-green completion and permission-seeking stop loops |

Scorecard artifacts are generated by
`base/yarn-ts/scripts/eval-harness-scorecard.ts`; rollout decisions are produced
by `base/yarn-ts/scripts/eval-rollback-policy.ts`. Main and release CI lanes
hold rollout after consecutive red-line breaches, while nightly runs produce
advisory decisions. Operator commands and exact artifact names live in
[`../development/TESTING.md#97-harness-trust-kpi-lane-coder-reliability`](../development/TESTING.md#97-harness-trust-kpi-lane-coder-reliability).
