...
- **Tool loop (in one `session/prompt`)**: The bridge advertises model tools based on the ACP capabilities negotiated during `initialize`: **Read** when the client supports **`fs/read_text_file`**, **Write** when it supports **`fs/write_text_file`**, and **Bash** when it supports **`terminal/create`**. When the model returns OpenAI `tool_calls`, the bridge emits ACP `tool_call` / `tool_call_update`, executes the call against the editor, appends `role: "tool"` messages, and **calls `/v1/chat/completions` again** until the model stops requesting tools (bounded rounds). Other tool names return a structured “not executed locally” result; advertised tools whose client capability disappears return a structured missing-capability result.
