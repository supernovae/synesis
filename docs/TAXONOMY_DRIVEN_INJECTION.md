...
| **Executor** | Uses `get_executor_depth_block(metadata)` to append depth instructions. |
| **Critic** | For `explain_only` + `taxonomy_metadata` (complexity > 0.6), validates that the response is "science-y" enough: required_elements covered, depth meets complexity. Routes to Router for revision when insufficient. |
...
7. **Executor** appends `get_executor_depth_block(metadata)` when present.
8. **Plan steps stream to UI** via existing SSE status events (main.py emits each step as `status`).
9. **Critic (document path):** When `explain_only` + `taxonomy_metadata`, the critic uses universal principles + taxonomy-as-hints to evaluate whether the response meets required depth. If insufficient → `approved=false` → Router → Executor revision loop.
...
7. Executor: Receives execution_plan; appends taxonomy depth block. Produces markdown structured by the plan.
...
- **Code path** survives: taxonomy shapes prompts but code decomposition (Planner) and execution (Executor) remain unchanged. Document path gains optional Planner when taxonomy requests deep-dive.
