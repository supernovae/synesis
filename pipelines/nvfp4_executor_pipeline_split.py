# Synesis NVFP4 Executor Pipeline (Split) — quant to PVC, then build, avoids Kaniko OOM.
#
# 70B model is huge; Kaniko holding quant output in memory causes OOM. This variant:
# 1. Quant: NVFP4 to KFP artifact (unchanged)
# 2. Copy: artifact → PVC at /data/executor-model
# 3. Build: Kaniko from PVC (single-stage, ~24Gi vs 80Gi+)
#
# Resume: If build fails, run executor-split-build-only — skips quant/copy, builds from existing PVC.
#
# Prereq: PVC (executor needs more space — 150Gi for 70B quantized)
#   sed "s/NAMESPACE/<ds-project>/" pipelines/manifests/executor-build-pvc.yaml | oc apply -f -
#
# Invoke: ./scripts/run-pipelines.sh executor-split
# Resume: ./scripts/run-pipelines.sh executor-split-build-only
...
def copy_artifact_to_pvc(
    ...
    """Copy quant output to PVC so Kaniko can build from it without holding in memory."""
...
@dsl.pipeline(
    ...
    description="Quantize 70B with NVFP4, copy to PVC, build ModelCar — avoids OOM",
)
def nvfp4_executor_pipeline_split(
...
@dsl.pipeline(
    ...
    description="Build only — assumes executor model already on PVC (resume after failed build)",
)
def nvfp4_executor_pipeline_split_build_only(
    ecr_uri: str = "123456789012.dkr.ecr.us-east-1.amazonaws.com/synesis-models",
    image_tag: str = "executor-nvfp4",
    pvc_name: str = "executor-build-pvc",
):
    """Resume: run after executor-split quant+copy succeeded but build failed."""
    build_task = build_and_push_modelcar(ecr_uri=ecr_uri, image_tag=image_tag)
    kubernetes.mount_pvc(
        build_task,
        pvc_name=pvc_name,
...
if __name__ == "__main__":
    ...
    compiler.Compiler().compile(
        pipeline_func=nvfp4_executor_pipeline_split,
        package_path=__file__.replace(".py", ".yaml"),
    )
    compiler.Compiler().compile(
        pipeline_func=nvfp4_executor_pipeline_split_build_only,
        package_path=__file__.replace(".py", "_build_only.yaml"),
    )
    print("Compiled to nvfp4_executor_pipeline_split.yaml and _build_only.yaml")
