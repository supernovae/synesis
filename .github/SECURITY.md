...
### pip-audit: indexer lockfile and crawl4ai dep tree

Crawl4AI's transitive dependency tree (120+ packages) breaks pip's backtracking resolver. The indexer pip-audit job uses a pre-compiled lockfile (`requirements-lock.txt`, generated by `uv pip compile`) instead of the loose `requirements.txt`. When updating `crawl4ai` or other indexer deps, regenerate:

```bash
uv pip compile base/rag/indexer/requirements.txt --python 3.11 \
  -o base/rag/indexer/requirements-lock.txt
```
