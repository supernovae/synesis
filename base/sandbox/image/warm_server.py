    ...
    def do_POST(self):
        ...
        try:
            ...
            if not _SAFE_FILENAME_RE.match(safe_filename):
                self._respond(400, {"error": "invalid filename", "exit_code": 1})
                return
            ...
            # Use work_dir for sandbox-work (writable); /tmp may have permission issues with emptyDir
            sandbox_work = os.path.join(work_dir, "sandbox-work")
            os.makedirs(sandbox_work, exist_ok=True)
            env["SANDBOX_WORK_DIR"] = sandbox_work
            ...
            # Log execute result so kubectl logs <warm-pool-pod> shows what happened
            ec = output.get("exit_code", -1)
            if ec != 0:
                lint = output.get("lint", {}) or {}
                sec = output.get("security", {}) or {}
                execo = output.get("execution", {}) or {}
