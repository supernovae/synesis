...
function toRagResult(row: Record<string, unknown>, fallbackScore: number): RagResult {
  return {
    ...
    pack_id: String(row.pack_id ?? "global"),
    pack_version: String(row.pack_version ?? ""),
    pack_source_version: String(row.pack_source_version ?? ""),
    pack_artifact_hash: String(row.pack_artifact_hash ?? ""),
    ...
    perf_tier: String(row.perf_tier ?? ""),
    safety_contract: String(row.safety_contract ?? ""),
    lifecycle_model: String(row.lifecycle_model ?? ""),
