...
async def recover_empty_messages_from_db(
    metadata: dict, original_messages: list[dict]
...
async def process_chat_payload(request, form_data, user, metadata, model):
    ...
    if not form_data["messages"]:
        ...
        if recovered_messages:
            form_data["messages"] = process_messages_with_output(recovered_messages)
            form_data["messages"] = strip_empty_content_blocks(form_data.get("messages", []))
            form_data["messages"] = merge_system_messages(form_data.get("messages", []))
            log.warning(
                "Recovered empty outbound chat messages from %s for chat_id=%s message_id=%s user_message_id=%s",
                recovery_source,
            ...
            log.warning(
                "Refusing to send empty outbound chat messages for model=%s chat_id=%s message_id=%s user_message_id=%s",
                form_data.get("model"),
                metadata.get("chat_id"),
                metadata.get("message_id"),
                metadata.get("user_message_id"),
...
async def streaming_chat_response_handler(response, ctx):
    ...
    filter_ids = await get_sorted_filter_ids(request, model, metadata.get("filter_ids", []))
    filter_functions = [await Functions.get_function_by_id(filter_id) for filter_id in filter_ids]
    ...
    if event_emitter and event_caller:
        ...
        async def response_handler(response, events):
            ...
            try:
                            ...
                            if data:
                                ...
                                # Check for Responses API events (type field starts with "response.")
                                    ...
                                    if raw_usage:
                                        ...
                                        # Synesis planner-ts: final SSE chunk may include run_id / authz_trace_id
                                        if data.get("run_id"):
                                            synesis_run_id = data["run_id"]
                                            emit_data["run_id"] = data["run_id"]
                                        if data.get("authz_trace_id"):
                                            synesis_authz_trace_id = data["authz_trace_id"]
                                            emit_data["authz_trace_id"] = data["authz_trace_id"]
                                        await event_emitter(
                                            {
                                                "type": "chat:completion",
                                                "data": emit_data,
                                        ...
                                        if data.get("run_id") or data.get("authz_trace_id"):
                                            merge_td = {}
                                            if data.get("run_id"):
                                                merge_td["synesis_run_id"] = data["run_id"]
                                            if data.get("authz_trace_id"):
                                                merge_td["synesis_authz_trace_id"] = data["authz_trace_id"]
                                            await Chats.upsert_message_to_chat_by_id_and_message_id(
                                                metadata["chat_id"],
                                                metadata["message_id"],
                                                merge_td,
                ...
                if synesis_run_id:
                    data["run_id"] = synesis_run_id
                if synesis_authz_trace_id:
                    data["authz_trace_id"] = synesis_authz_trace_id

                synesis_msg_extra = {}
                if synesis_run_id:
                    synesis_msg_extra["synesis_run_id"] = synesis_run_id
                if synesis_authz_trace_id:
                    synesis_msg_extra["synesis_authz_trace_id"] = synesis_authz_trace_id
