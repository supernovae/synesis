...
def test_custom_provider_inherits_governance_key_endpoint_prefix() -> None:
    ...
    assert r.litellm_params["api_key"] == "os.environ/ACME_API_KEY"
    assert r.litellm_params["api_base"] == "https://api.acme/v1"
    assert r.litellm_params["model"] == "openai/gpt-4"


def test_governance_litellm_prefix_overrides_custom_catalog_default() -> None:
    ...
    maps = ProviderGovernanceMaps(
        ...
        litellm_prefixes={"weird": "vertex_ai/"},
    )
    r = resolve_deployment_routing_for_parts(
        provider="weird",
        model="gemini-pro",
    ...
    assert r.litellm_params["model"] == "vertex_ai/gemini-pro"
...
def test_role_api_key_env_override_wins_over_governance() -> None:
    ...
    assert r.litellm_params["api_key"] == "os.environ/MY_OPENROUTER"
