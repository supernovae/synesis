    ...
    def fetch(self, source_config: dict[str, Any]) -> list[RawDocument]:
        ...
        try:
            ...
            return _collect_source_files(
                tmpdir, repo, branch, language, paths, extensions, name,
            )
    ...
    def parse_and_chunk(self, doc: RawDocument) -> list[Chunk]:
        ...
        return [
            Chunk(
                ...
                heading_path=f"{file_path} > {cc.get('symbol_name', '')}".strip(" > "),
                chunk_index=i,
                metadata={
        ...
        if len(text) <= MAX_CHUNK_CHARS:
            ...
            for child in node.children:
                if child.type in config.get("nested_types", set()):
                    ...
                    child_text = (child_leading + source_bytes[child.start_byte : child.end_byte]).decode("utf-8", errors="replace")
