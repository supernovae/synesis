...
def _should_use_bulk_import(job: dict[str, Any], pack_path: Path) -> bool:
    ...
    if _IMPORT_BACKEND == "legacy-bolt":
        raise RuntimeError("legacy-bolt content pack import is no longer supported; rebuild as SynPack v2")
    if _IMPORT_BACKEND != "auto":
        raise RuntimeError(f"unsupported content pack import backend: {_IMPORT_BACKEND}")
    if not _is_v2_pack(pack_path):
        raise RuntimeError("content pack install requires SynPack v2 typed nodes/chunks.jsonl")
    return True
...
def _load_content_pack(job: dict[str, Any], pack_path: Path, *, nornic_uri: str) -> dict[str, Any]:
    ...
    raise RuntimeError("content pack install requires the SynPack v2 bulk importer")
