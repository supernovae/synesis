"""Unified indexing pipeline: fetch -> parse -> chunk -> enrich -> embed -> upsert.
...
from .schema import SYNESIS_CATALOG, catalog_entity, ensure_synesis_catalog
...
def index_source(
    ...
    *,
    ...
    logger.info(
        ...
        len(new_chunks), len(all_chunks) - len(new_chunks), name,
    )
    ...
    # 6. Build catalog entities
    entities = []
    for (doc, chunk, cid), enrichment, emb in zip(new_chunks, enrichments, embeddings):
        chunk_tags = chunk.metadata.get("tags", tags_str)
        chunk_source_url = chunk.metadata.get("source_url", doc.source_url)
