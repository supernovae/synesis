...
def test_normalization_does_not_delete_legitimate_lines_or_code_spacing():
    original = "Home\nSearch\n```\nclose\n\n\ncall()\n```"
    assert extract_mod.normalize_doc_markdown(original) == original
...
def test_saved_html_round_trip_through_real_pack_cli(tmp_path):
    ...
    extraction = subprocess.run(
        [
            sys.executable,
            str(ROOT / "base/rag/indexer/app/extract.py"),
            "--root",
            str(tmp_path),
            "--input",
            source.name,
