...
export function createLicenseCheckTool() {
    return {
        name: "synesis_license_check",
        description: "Check open-source license compatibility. Given package SPDX licenses, returns per-package compatibility and conflicts against a target license.",
        inputSchema: {
            type: "object",
            properties: {
                packages: {
        ...
        handler: async (args) => {
            try {
                ...
                const target = args.target_license === undefined || args.target_license === null
                    ? "Apache-2.0"
                ...
                for (const raw of rawPackages) {
                    ...
                    const lic = pkg.license === undefined || pkg.license === null ? "UNKNOWN" : String(pkg.license);
