...
function buildSearchBody(args, caller, fixedArtifactKind) {
    ...
    if (caller?.org_id)
        body.caller_org_id = caller.org_id;
    if (caller?.tenant_ids?.length)
        body.caller_tenant_ids = [...caller.tenant_ids];
    if (caller?.acl_groups?.length)
        body.caller_acl_groups = [...caller.acl_groups];
    if (caller?.user_id)
        body.caller_user_id = caller.user_id;
    return body;
}
async function runKnowledgeSearch(config, args, caller, fixedArtifactKind) {
    try {
        const query = String(args.query ?? "").trim();
        if (!query) {
...
export function createKnowledgeSearchTools(config) {
    return [
        {
            name: "synesis_search",
            description: "RAG retrieval against the Synesis knowledge catalog. Returns ranked chunks with provenance and scores. Supports metadata filters (language, artifact kind, domain, tags, repo path, etc.).",
            inputSchema: searchInputSchema,
            handler: async (args, caller) => runKnowledgeSearch(config, args, caller, undefined),
        },
        {
            name: "synesis_code_search",
            description: "Search the Synesis code corpus (artifact_kind=code). Returns ranked code chunks with provenance.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Search the Synesis documentation corpus (artifact_kind=docs). Returns ranked documentation chunks.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        },
        {
            ...
            description: "Search the Synesis configuration corpus (artifact_kind=config): YAML, JSON, HCL, Kubernetes, Terraform, CI, etc.",
            inputSchema: {
                type: "object",
                properties: {
