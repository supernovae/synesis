...
export function createDocsLookupTool() {
    return {
        name: "synesis_docs_lookup",
        description: "Look up versioned documentation for curated frameworks (fastapi, langchain, langgraph, vllm, react, kubernetes, openshift): doc URLs, API references, changelogs, and migration notes.",
        inputSchema: {
            type: "object",
            properties: {
                framework: {
        ...
        handler: async (args) => {
            try {
                const framework = String(args.framework ?? "")
                    .toLowerCase()
                    .trim();
                const version = args.version === undefined || args.version === null
                    ? "latest"
                    : String(args.version);
                const topic = args.topic === undefined || args.topic === null ? "" : String(args.topic);
                if (!framework) {
                ...
                const docInfo = versions[version] ??
                    versions.latest ??
                    {};
                const result = {
                    found: true,
                ...
                if (docInfo.changelog !== undefined)
                    result.changelog = docInfo.changelog;
                if (docInfo.recipes !== undefined)
                    result.recipes = docInfo.recipes;
...
//# sourceMappingURL=docs-lookup.js.map