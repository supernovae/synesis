...
function extractPlanContent(data) {
...
export function createPlanTool(config) {
    return {
        name: "synesis_plan",
        description: "Generate an execution plan for a complex task. Returns structured steps and narrative plan text in the `plan` field.",
        inputSchema: {
            type: "object",
            properties: {
        ...
        handler: async (args) => {
            try {
                ...
                const context = args.context === undefined || args.context === null
                    ? ""
                    : String(args.context);
                let prompt = task;
                if (context.trim()) {
