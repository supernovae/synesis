...
} from "../src/streaming/claude-stream-route-completion.js";
...
describe("completeClaudeStreamRoute", () => {
  it("builds completion input from route finalizer and telemetry factories", () => {
    const persistDecisionTelemetry = vi.fn();
    const recordSessionEvent = vi.fn();
    const routeInput = createClaudeStreamRouteCompletionInput({
      finalizer: {
    ...
    expect(routeInput.finalizerInput.handlers).toEqual({
      finalizePendingText: expect.any(Function),
      finalizeHistoryText: expect.any(Function),
    });
    expect(routeInput.telemetryBase).toMatchObject({
      requestId: "req_1",
      sessionKey: "session_1",
      userId: "user_1",
      orgId: "org_1",
    ...
    expect(routeInput.toolNames).toEqual(["Edit"]);
  });
