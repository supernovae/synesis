 ...
 */
const CLAUDE_CODE_PARAM_ALIASES = {
...
export class Qwen3CoderAdapter {
    family = "qwen3-coder";
...
export class GenericOpenAIAdapter {
    family;
    supportsThinking = false;
 ...
 */
export function repairBashToolCall(toolName, input) {
    if (toolName !== "Bash")
        return null;
    ...
    const nonEmptyStringPairs = extras.filter(([, v]) => typeof v === "string" && v.trim());
    ...
    if (nonEmptyStringPairs.length === 1) {
        ...
        if (typeof input.is_background === "boolean")
            out.is_background = input.is_background;
    ...
    if (extras.length === 1 && nonEmptyStringPairs.length === 0) {
        ...
        if (v === "" || v === null || v === undefined) {
            const msg = "Synesis Yarn: model sent invalid Bash arguments (no command string). " +
...
export function repairWriteToolCall(toolName, input) {
    if (toolName !== "Write")
        return null;
    ...
    if (looksLikePythonDict)
        return null;
    if (!tooShortForCode && !noNewlinesInCode)
        return null;
...
export function normalizeFileToolArgs(toolName, input) {
 ...
 */
export function constrainFileToolPathToProjectRoot(projectRoot, toolName, input) {
    if (!projectRoot?.trim())
 ...
 */
export function normalizeHallucinatedLinuxWritePath(filePath) {
 ...
 * - leading "./"
 */
export function normalizeWorkspaceRelativeFilePath(filePath) {
    let p = filePath.trim();
    if (!p)
        return p;
    p = p.replace(/^["'`]+|["'`]+$/g, "");
 ...
 */
function hasNativeQwenToolParser(baseUrl) {
    if (!baseUrl)
        return false;
    ...
    if (u.includes(".svc.cluster.local") || u.includes("localhost") || u.includes("127.0.0.1"))
        return true;
    return false;
}
/**
 ...
 */
export function resolveAdapter(backendModel, baseUrl) {
    const m = backendModel.toLowerCase();
    if (/qwen3.*coder/i.test(m))
        return new Qwen3CoderAdapter(hasNativeQwenToolParser(baseUrl));
    if (/deepseek/i.test(m))
        return new DeepSeekAdapter();
    if (/kimi|moonshot/i.test(m))
        return new GenericOpenAIAdapter("kimi");
    if (/minimax|abab/i.test(m))
        return new GenericOpenAIAdapter("minimax");
