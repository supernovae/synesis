import { z } from "zod";
import { FALLBACK_BASE_RATES, hasNonZeroRates, } from "@synesis/telemetry";
...
export function resolveTierRates(registryRates, registrySource) {
...
export async function fetchTierRegistrySnapshot(config) {
    ...
    for (const row of payload.roles) {
        ...
        const registryRates = {
            ...
            cached_input_per_million: cost?.input_cached_per_million == null
                ? null
                : Number(cost.input_cached_per_million),
        };
        const { rates, pricingSource } = resolveTierRates(registryRates, cost?.pricing_source);
        out.push({
            id: tierId,
            backendModel: row.model ?? "",
            baseUrl: endpoint,
...
export async function fetchTierConfigs(config) {
