 ...
 */
export function formatSyntheticResponse(resolution) {
    const lines = [
    ...
    for (let i = 0; i < resolution.findings.length; i++) {
        ...
        if (f.errorFamily !== "unknown")
            lines.push(`   Error family: ${f.errorFamily}`);
        if (f.rootCause)
            lines.push(`   Root cause: ${f.rootCause}`);
        if (f.recipe) {
            lines.push(`   Fix: ${f.recipe.template}`);
            if (f.recipe.description)
                lines.push(`   Detail: ${f.recipe.description}`);
            if (f.recipe.steps?.length) {
                lines.push(`   Steps: ${f.recipe.steps.join(" → ")}`);
            }
            if (f.recipe.constraints)
                lines.push(`   Constraints: ${f.recipe.constraints}`);
        }
        if (f.action)
 ...
 */
export function formatEnrichmentBlock(resolution) {
...
function formatFindingHint(f) {
    const parts = [`- [${f.errorFamily}]`];
    if (f.rootCause)
        parts.push(`cause: ${f.rootCause}`);
    if (f.recipe)
        parts.push(`fix: ${f.recipe.template}`);
    return parts.join(" | ");
}
/**
 ...
 */
export function formatSelfRepairBlock(resolution, loopState) {
    const withRecipes = resolution.findings.filter((f) => f.recipe);
