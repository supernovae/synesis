...
import { createServiceMetrics, recordUsageMetrics, computeCost, emitTrace, } from "@synesis/telemetry";
import { loadConfig } from "./config.js";
import { ClaudeMessagesRequestSchema, OpenAIChatCompletionRequestSchema } from "./schemas.js";
import { fetchTierRegistrySnapshot, TIER_TO_ROLE, } from "./providers/admin-tier-registry.js";
...
import { registerToolCollapseRoutes, ToolCallInterceptor, planToSyntheticToolCalls, defaultShellAllowlistFromEnv, } from "./tool-collapse/index.js";
...
import { appendPathContextToAdapterBlock, ClientAdapterPacks, parseSessionExecutionContext, resolveWorkspaceRootForCollapse, } from "./adapters/client-adapter-packs.js";
...
import { extractToolSchemaName, pruneToolSchemas, } from "./compat/tool-schema-pruning.js";
...
import { buildDecisionSnapshot, snapshotToTraceFields } from "./telemetry/decision-snapshot.js";
import { analyzeGaps, shouldTriggerSensemaking, buildExplorationPlan, formatExplorationPlanBlock, createEmptySensemakingStats, } from "./sensemaking/index.js";
...
function getMetadataString(meta, key) {
    const value = meta[key];
    return typeof value === "string" ? value : "";
}
function trimSnippet(text, max = 2000) {
    return text.replace(/\s+/g, " ").trim().slice(0, max);
}
function updateTracePromptMetadata(state, latestUserText) {
...
function buildRequirementChecklistSnapshot(checklist) {
    return {
...
function refreshRequirementChecklist(state) {
...
function getChecklistSourceHash(meta) {
    const row = meta.requirement_checklist;
    if (!row || typeof row !== "object")
        return "";
    const value = row.sourceHash;
    return typeof value === "string" ? value : "";
}
function buildCompletionGapMessage(missingSummary) {
    return [
...
function completionCriticBlock(checklist) {
...
function appendCriticBlock(messages, checklist) {
    if (!checklist || (checklist.must.length === 0 && checklist.should.length === 0))
        return messages;
...
function extractRecentToolNames(messages) {
    ...
    for (const m of messages) {
        if (m.role !== "assistant" || !m || typeof m !== "object")
            continue;
        const row = m;
...
function extractRequestedToolNames(userText, tools) {
...
function parseTierCFallbackJson(raw, maxFindings) {
...
function pushDiagnostic(d) {
    ...
    if (diagnosticRing.length > DIAGNOSTIC_RING_MAX)
        diagnosticRing.shift();
    if (d.requestId) {
        diagnosticStore.persistDiagnostic(d.requestId, d);
    }
}
...
const yarnDedupeLayer = config.SYNESIS_YARN_TOOL_COLLAPSE_ENABLED && config.SYNESIS_YARN_DEDUPE_ENABLED
    ? new DedupeLayer({
        ...
        log: (e) => app.log.info({ kind: e.kind, msg: e.message, toolCallIds: e.toolCallIds, detail: e.detail }, "yarn_dedupe"),
    })
    : null;
const yarnToolPrefixCache = config.SYNESIS_YARN_TOOL_COLLAPSE_ENABLED && config.SYNESIS_YARN_TOOL_PREFIX_CACHE_ENABLED
    ? new ToolPrefixCache({
...
const roleAssignmentRegistry = new Map();
let promptSnapshotRegistry = null;
...
const usagePersistenceEnabled = config.SYNESIS_YARN_PERSIST_USAGE_TO_DB && Boolean(String(config.SYNESIS_YARN_ADMIN_DB_URL ?? "").trim());
if (!usagePersistenceEnabled) {
    ...
    if (config.SYNESIS_YARN_CONVERSATION_MEMORY_ENABLED) {
        void usageWriter.ensureContinuityTable().catch((err) => app.log.warn({ err }, "Failed to ensure yarn_session_continuity table (non-fatal)"));
    }
}
...
function inferModelFamily(backendModel) {
    const m = (backendModel || "").toLowerCase();
    if (/qwen3.*coder/.test(m))
        return "qwen3-coder";
    if (/deepseek/.test(m))
        return "deepseek";
    if (/kimi|moonshot/.test(m))
        return "kimi";
    return "generic";
}
function enrichWithFrameAndManifest(messages, sessionKey, adapterBlock, promptContext, pathHints) {
    const out = [...messages];
    let detectedPhase;
    let detectedGoal;
    ...
    const wfPathHints = config.SYNESIS_YARN_SESSION_PATH_HINTS_IN_WORKING_FRAME && pathHints
    ...
    if (config.SYNESIS_YARN_WORKING_FRAME_ENABLED) {
        if (config.SYNESIS_YARN_MANIFEST_TEMPLATES_ENABLED) {
            ...
            if (complexityResult.complexity === "tiny" || complexityResult.complexity === "small") {
                ...
                const filePaths = (allText.match(FILE_RE_GLOBAL) ?? []).map((f) => f.trim());
                const observed = manifestScan({ filePaths, conversationText: allText });
                const manifestCtx = { complexity: complexityResult.complexity };
                if (template) {
    ...
    if (config.SYNESIS_YARN_VERIFICATION_PLAN_ENABLED) {
        ...
        if (detectedLangs.length > 0) {
            const vPlan = buildVerificationPlan(detectedLangs, getLanguagePackRegistry(), config.SYNESIS_YARN_VERIFICATION_MAX_ROUNDS, config.SYNESIS_YARN_VERIFICATION_BUDGET_MS);
...
function phaseFromFrame(currentPhase) {
...
function detectLanguagesFromMessages(messages) {
...
function buildSessionKey(userId, clientKind, conversationId) {
 ...
 */
async function getSessionKey(identity) {
    ...
    if (inMemory) {
        ...
        if (idle > config.SYNESIS_YARN_SESSION_INACTIVITY_ROTATION_MS) {
            app.log.info({ oldKey: baseKey, idleMs: idle, tokens: inMemory.record.totalTokensIn }, "session_inactivity_rotation");
    ...
    if (loaded) {
        ...
        if (idle > config.SYNESIS_YARN_SESSION_INACTIVITY_ROTATION_MS) {
            app.log.info({ oldKey: baseKey, idleMs: idle, tokens: loaded.totalTokensIn }, "session_inactivity_rotation_redis");
            const rotated = `${baseKey}:r${Date.now()}`;
            return rotated;
        }
    }
...
async function getSessionState(key, identity) {
...
async function casSessionSave(state) {
    try {
...
function injectSessionContext(messages, state) {
    const compacted = state.history.find((m) => m.role === "system" && m.content.includes("<ARCHITECTURAL_STATE>"));
    if (!compacted)
        return messages;
    ...
    try {
        ...
        if (snapshot.tiers.length > 0) {
            ...
            for (const t of snapshot.tiers) {
                if (!t.apiKey?.trim()) {
                    app.log.warn({ tier: t.id, baseUrl: t.baseUrl, backendModel: t.backendModel }, "tier_missing_api_key_env — set the key in provider-api-keys secret (same namespace as yarn) or SYNESIS_YARN_OPENAI_COMPAT_API_KEY");
                }
            }
        }
        else {
            app.log.warn({}, "tier_registry_empty — no assigned coder-pulse / coder-core / coder-horizon / coder-compaction roles in admin, or role fetch returned none");
        }
        const compactionTier = tierRegistry.getTierConfig("synesis-compaction");
        if (compactionTier) {
            sawtooth.setCompactFn(async (system, userPrompt) => {
...
async function runValidationTierCFallback(ctx) {
    if (!config.SYNESIS_YARN_VALIDATION_TIER_C_ENABLED)
        return null;
    const role = config.SYNESIS_YARN_VALIDATION_TIER_C_ROLE;
    const assigned = roleAssignmentRegistry.get(role);
    if (!assigned?.assigned || !assigned.backendModel)
        return null;
    ...
    try {
        const { model } = tierRegistry.resolveAdHoc(`synesis-tierc-${role}`, assigned.backendModel, assigned.baseUrl, assigned.apiKey);
        const result = await generateText({
            model: model,
            maxOutputTokens: 700,
            messages: [
...
import { repairBashToolCall, repairWriteToolCall, } from "./providers/model-adapter.js";
import { governToolCall } from "./path-governance/tool-call-governance.js";
import { buildWorkspaceHandshakeBashCommand, contextFromSessionMetadata, extractClaudeToolResult, extractOpenAIToolResult, hasBashTool, makeWorkspaceHandshakeToolCallId, parseWorkspaceContextOutput, } from "./session/workspace-context-handshake.js";
function runOpenAIRequest(request) {
    try {
        const resolved = tierRegistry.resolve(request.model, config.SYNESIS_YARN_DEFAULT_TIER);
        const sanitized = sanitizeToolCalls(request.messages);
...
function persistSessionAndUsage(state, requestId, resolvedModelId, usage, latencyMs, finishReason, tokensSavedByReduction = 0, escalated = false, snapshot) {
    ...
    let pricingSource = tier?.pricingSource ?? "unknown";
    let computedCostUsd;
    if (usage.costUsd > 0) {
    ...
    void distributedCounters.setConsecutiveToolCalls(state.record.sessionKey, state.consecutiveToolCalls);
    ...
    if (config.SYNESIS_YARN_CONVERSATION_MEMORY_ENABLED && state.record.continuity) {
        usageWriter.enqueueContinuityUpsert(state.record.userId, state.record.orgId, state.record.sessionKey, state.record.continuity);
    }
...
function readUsage(input) {
    ...
    if (!cached) {
        const inputTokenDetails = obj.inputTokenDetails;
...
function resolveClaudeConversationId(metadata, headers) {
    if (metadata) {
...
function normalizeForSignal(value) {
    ...
    if (Array.isArray(value))
        return value.map((v) => normalizeForSignal(v));
    const out = {};
    const keys = Object.keys(value).sort();
    for (const key of keys)
        out[key] = normalizeForSignal(value[key]);
    return out;
}
function stableSignalString(value) {
    if (typeof value === "string") {
        return value.replace(/\s+/g, " ").trim();
    }
    return JSON.stringify(normalizeForSignal(value));
}
function hashTextSignal(value) {
...
function detectToolProgress(session, messages) {
...
function toolLoopSoftFailMessage(decision) {
...
function setSessionWorkspaceContext(state, status, reqId, details) {
    ...
    if (details?.cwd)
        state.record.metadata.workspace_context_cwd = details.cwd;
    if (details?.projectRoot)
        state.record.metadata.workspace_context_project_root = details.projectRoot;
    if (details?.shell)
        state.record.metadata.workspace_context_shell = details.shell;
    if (details?.os)
        state.record.metadata.workspace_context_os = details.os;
    if (details?.arch)
        state.record.metadata.workspace_context_arch = details.arch;
...
function sendClaudeWorkspaceHandshake(reply, model, stream, toolCallId) {
    const input = {
        command: buildWorkspaceHandshakeBashCommand(),
        description: "Initializing workspace context (read-only): cwd/project root/shell/os",
    };
...
function sendOpenAISoftFail(reply, requestId, model, content, stream) {
    if (!stream) {
        return reply.send({
            id: requestId,
            object: "chat.completion",
...
function sendClaudeSoftFail(reply, model, content, stream) {
    if (!stream) {
        return reply.send({
            id: `msg_${crypto.randomUUID()}`,
            type: "message",
...
function logAndPersistSafetyEvent(decision, sessionKey, sessionTokensIn) {
...
function recordSessionEvent(sessionKey, userId, orgId, eventKind, component, detail, requestId, meta) {
...
function emitDecisionEvents(sessionKey, userId, orgId, requestId, snapshot) {
    if (!snapshot || !config.SYNESIS_YARN_DECISION_MATRIX_ENABLED)
        return;
    recordSessionEvent(sessionKey, userId, orgId, "decision_routing", "phase-model-orchestrator", `${snapshot.decisionPath} → ${snapshot.tier} (${snapshot.phase})`, requestId, {
    ...
    if (snapshot.escalated) {
        recordSessionEvent(sessionKey, userId, orgId, "escalation", "phase-model-orchestrator", snapshot.escalationReason ?? "escalated", requestId, {
            tier: snapshot.tier,
            phase: snapshot.phase,
            recallRouting: snapshot.recallRouting,
    ...
    if (snapshot.sensemakingTriggered && config.SYNESIS_YARN_SENSEMAKING_ENABLED) {
        recordSessionEvent(sessionKey, userId, orgId, "sensemaking_triggered", "sensemaking-engine", snapshot.sensemakingReason ?? "sensemaking", requestId, {
            phase: snapshot.phase,
            decisionPath: snapshot.decisionPath,
...
async function proxyMcpGet(path, bearer, headers) {
    try {
...
async function proxyMcpPost(path, bearer, body, headers) {
    try {
        const response = await fetch(`${config.SYNESIS_YARN_ADMIN_API_URL}${path}`, {
            method: "POST",
            signal: AbortSignal.timeout(config.SYNESIS_YARN_MCP_PROXY_TIMEOUT_MS),
...
function sse(reply, event, data) {
    reply.raw.write(`event: ${event}\ndata: ${JSON.stringify(data)}\n\n`);
}
function safeWrite(raw, data) {
    try {
        if (raw.destroyed)
            return false;
        raw.write(data);
...
function safeSse(reply, event, data) {
    return safeWrite(reply.raw, `event: ${event}\ndata: ${JSON.stringify(data)}\n\n`);
}
function safeEnd(raw) {
    try {
        if (!raw.destroyed)
            raw.end();
    }
...
function sanitizeUpstreamError(err) {
    const raw = err instanceof Error ? err.message : String(err);
    if (/timed?\s*out/i.test(raw))
        return "Upstream model request timed out";
    if (/ECONNREFUSED|ENOTFOUND|ECONNRESET|socket hang up/i.test(raw))
        return "Upstream model service unavailable";
    if (/\b[45]\d{2}\b/.test(raw))
        return "Upstream model service error";
    if (/rate.?limit/i.test(raw))
        return "Upstream rate limit exceeded";
    if (/context.?length|too.?long|too.?large/i.test(raw))
 ...
 */
function claudeSystemToMessage(system) {
    if (!system)
        return null;
...
function debugProtocolLog(logger, reqId, path, extra) {
    if (!config.SYNESIS_YARN_DEBUG_PROTOCOL)
        return;
...
app.post("/v1/chat/completions", async (req, reply) => {
    ...
    if (!oaiRateResult.allowed) {
        ...
        recordSessionEvent("", authUser.userId, authUser.orgId, "rate_limit_reject", "user-rate-limiter", `${oaiRateResult.currentCount}/${oaiRateResult.limit} in window — retry after ${oaiRateResult.retryAfterSeconds}s`);
    ...
    const oaiTraceReqId = resolveRequestId(req.headers);
    // Sorted tools for cache stability
    if (config.SYNESIS_YARN_SORTED_TOOLS_ENABLED && request.tools) {
        request.tools = sortToolSchemas(request.tools);
    }
    const reducedOpenAI = enrichmentPool.isAvailable()
        ? await withSpanAsync("yarn.enrichment", { "yarn.path": "openai" }, () => toolResultReduction.reduceMessagesAsync(request.messages, enrichmentPool))
        : withSpan("yarn.enrichment", { "yarn.path": "openai" }, () => toolResultReduction.reduceMessages(request.messages));
    ...
    const oaiBodyMeta = oaiBodyMetaRaw && typeof oaiBodyMetaRaw === "object" && !Array.isArray(oaiBodyMetaRaw)
        ? oaiBodyMetaRaw
        : null;
    const oaiPathCtx = parseSessionExecutionContext(req.headers, oaiBodyMeta);
    const adapterBlock = appendPathContextToAdapterBlock(clientAdapterPacks.toSystemBlock(adapterProfile), req.headers, oaiBodyMeta, String(req.headers["x-synesis-client"] ?? ""));
    const latestUserText = [...normalizedOpenAI.messages].reverse().find((m) => m.role === "user");
    const preManifest = projectManifestService.build(normalizedOpenAI.messages);
    debugProtocolLog(app.log, oaiTraceReqId, "/v1/chat/completions", {
        model: request.model,
    ...
    const oaiClientKind = String(req.headers["x-synesis-client"] ?? "unknown");
    const identity = {
    ...
    if (oaiRequirementChecklist && oaiRequirementChecklist.sourceHash !== priorOaiChecklistHash) {
        recordSessionEvent(sessionKey, identity.userId, identity.orgId, "requirements_checklist", "completion-gate", `Checklist initialized (must=${oaiRequirementChecklist.must.length}, should=${oaiRequirementChecklist.should.length})`, oaiTraceReqId);
    }
    ...
    if (workspaceStatus === "pending" && pendingWorkspaceToolId) {
        const toolResult = extractOpenAIToolResult(request.messages, pendingWorkspaceToolId);
        if (toolResult !== null) {
    ...
    if (config.SYNESIS_YARN_EVIDENCE_PREFETCH_ENABLED && latestUserText) {
        ...
        if (prefetchText.length > 0) {
            oaiPrefetchResult = await runEvidencePrefetch(prefetchText, knowledgeSearch, config.SYNESIS_YARN_EVIDENCE_PREFETCH_TIMEOUT_MS, config.SYNESIS_YARN_EVIDENCE_CONFIDENCE_MIN, { retryEnabled: config.SYNESIS_YARN_EVIDENCE_PREFETCH_RETRY_ENABLED });
    ...
    let oaiPatternResult;
    if (config.SYNESIS_YARN_PATTERN_RECALL_ENABLED && latestUserText && !oaiPrefetchResult?.matched) {
        const prefetchText = typeof latestUserText.content === "string" ? latestUserText.content : "";
        if (prefetchText.length > 0) {
            oaiPatternResult = await runPatternPrefetch(prefetchText, knowledgeSearch, config.SYNESIS_YARN_EVIDENCE_PREFETCH_TIMEOUT_MS, oaiWorkingPhase);
    ...
    const combinedEvidenceConfidence = Math.max(oaiPrefetchResult?.confidence ?? 0, oaiPatternResult?.confidence ?? 0);
    ...
    const oaiLastToolId = [...request.messages]
    ...
    const oaiToolProgress = detectToolProgress(session, normalizedOpenAI.messages);
    ...
    if (!policyPrecheck.allow) {
        ...
        if (config.SYNESIS_YARN_TOOL_LOOP_SOFT_FAIL_ENABLED && policyPrecheck.softFailClass === "tool_loop") {
            ...
            persistSessionAndUsage(session, oaiTraceReqId, orchestration.selectedModel, usage, Date.now() - started, "stop", 0);
            maybeCheckpoint(session);
            recordSessionEvent(sessionKey, identity.userId, identity.orgId, "tool_loop_soft_fail", "deterministic-policy", policyPrecheck.rejectReason ?? "Tool loop soft fail", oaiTraceReqId);
    ...
    const oaiEnriched = enrichWithFrameAndManifest(normalizedOpenAI.messages, sessionKey, effectiveOaiAdapterBlock, oaiPromptContext, { projectRoot: effectiveOaiPathCtx.projectRoot, shellCwd: effectiveOaiPathCtx.shellCwd });
    let oaiEnrichedMsgs = appendCriticBlock(oaiEnriched.messages, oaiRequirementChecklist);
    let oaiSensemakingResult;
    ...
    if (config.SYNESIS_YARN_JITTER_BUFFER_ENABLED) {
        ...
        oaiEnrichedMsgs = applyJitter(stableMessages, jitterBlock);
    }
    ...
    oaiEnrichedMsgs = trustResult.messages;
    const normalizedRequest = {
        ...request,
        model: orchestration.selectedModel,
        messages: oaiEnrichedMsgs
    };
    ...
    normalizedRequest.messages = injectSessionContext(normalizedRequest.messages, session);
    if (config.SYNESIS_YARN_ARTIFACT_RETRIEVAL_ENABLED) {
        normalizedRequest.tools = artifactRetrieval.injectToolOpenAI(normalizedRequest.tools);
    }
    if (config.SYNESIS_YARN_KNOWLEDGE_SEARCH_ENABLED) {
        normalizedRequest.tools = knowledgeSearch.injectToolOpenAI(normalizedRequest.tools);
    }
    {
        const blocks = [];
        ...
        if (blocks.length > 0) {
            ...
            const msgs = normalizedRequest.messages;
    ...
    const sdkTools = openAIToolsToSDK(effectiveTools);
    ...
    const modelMessages = modelToolPrompt
        ? [{ role: "system", content: modelToolPrompt }, ...messages]
        : messages;
    const adapterProviderOptions = adapter.providerOptions?.();
    if (!normalizedRequest.stream) {
        if (!circuitBreakers.allowRequest(resolved.resolvedModelId, identity.orgId)) {
            ...
            recordSessionEvent(sessionKey, identity.userId, identity.orgId, "breaker_open_reject", "circuit-breaker", `Circuit breaker open for ${resolved.resolvedModelId}`, reqId, { model: resolved.resolvedModelId });
        ...
        try {
            ...
            for (let round = 0; round < 3; round++) {
                ...
                if (finalResult.text)
                    assistantParts.push({ type: "text", text: finalResult.text });
                for (const ac of serverCalls) {
        ...
        let externalToolCalls = toolCalls
            ...
            .map((tc) => {
            const rawInput = typeof tc.input === "object" && tc.input !== null && !Array.isArray(tc.input)
                ? tc.input
                : {};
        ...
        if (config.SYNESIS_YARN_TOOL_COLLAPSE_ENABLED &&
            ...
            const workspaceRoot = resolveWorkspaceRootForCollapse(req.headers, oaiBodyMeta);
        ...
        const usage = readUsage(finalResult.usage);
        ...
        const msgCounts = countMessageRoles(normalizedRequest.messages);
        pushDiagnostic({
        ...
        const message = { role: "assistant", content: finalAssistantText };
    ...
    if (!oaiAdmission.admitted) {
        ...
        recordSessionEvent(sessionKey, identity.userId, identity.orgId, "stream_admission_reject", "stream-admission", oaiAdmission.reason ?? "stream admission rejected", reqId);
    ...
    if (!circuitBreakers.allowRequest(resolved.resolvedModelId, identity.orgId)) {
        ...
        recordSessionEvent(sessionKey, identity.userId, identity.orgId, "breaker_open_reject", "circuit-breaker", `Circuit breaker open for ${resolved.resolvedModelId} (stream)`, reqId, { model: resolved.resolvedModelId });
        reply.header("Retry-After", "30");
        return reply.code(503).send({ error: { type: "service_unavailable", message: "Model provider temporarily unavailable. Try again shortly." } });
    }
    ...
    try {
        for await (const part of streamed.fullStream) {
            ...
            if (part.type === "text-delta") {
                pendingTextDeltas.push(part.text ?? "");
            }
                ...
                if (part.type === "tool-input-start") {
                    ...
                    if (adapter.normalizeToolCallArgs)
                        argsStr = adapter.normalizeToolCallArgs(argsStr);
                    const parsedInput = typeof tc.input === "object" && tc.input !== null && !Array.isArray(tc.input)
                        ? tc.input
                        : (() => {
                            try {
    ...
    let streamedText = "";
    try {
        oaiStreamUsage = readUsage(await streamed.totalUsage);
    }
    catch { /* stream aborted */ }
    try {
        streamedText = await streamed.text;
    }
    catch { /* stream aborted */ }
    if (streamedText) {
    ...
    const oaiStreamMsgCounts = countMessageRoles(normalizedRequest.messages);
    pushDiagnostic({
...
app.post("/v1/messages", async (req, reply) => {
    ...
    if (!claudeRateResult.allowed) {
        ...
        recordSessionEvent("", claudeAuthUser.userId, claudeAuthUser.orgId, "rate_limit_reject", "user-rate-limiter", `${claudeRateResult.currentCount}/${claudeRateResult.limit} in window — retry after ${claudeRateResult.retryAfterSeconds}s`);
    ...
    const traceReqId = resolveRequestId(req.headers);
    ...
    const rawOpenAIMessages = withSpan("yarn.enrichment", { "yarn.path": "claude" }, () => claudeMessagesToOpenAI(body.messages, (content, toolName) => toolResultReduction.reduceStandaloneToolResult(content, toolName)));
    ...
    const toolSearchResult = applyToolSearchPolicy(body.tools, config.SYNESIS_YARN_CLAUDE_TOOL_SEARCH_MODE);
    ...
    const claudeToolResultCount = body.messages.filter((m) => m.role === "tool_result" || m.role === "tool").length;
    const normalizedFromClaude = await validationNormalization.normalizeMessagesAsync(openAIMessages, runValidationTierCFallback);
    debugProtocolLog(app.log, traceReqId, "/v1/messages", {
        model: body.model,
    ...
    const claudeManifest = projectManifestService.build(normalizedFromClaude.messages);
    const claudeClientKind = String(req.headers["x-synesis-client"] ?? "claude-code");
    const claudeConversationId = resolveClaudeConversationId(body.metadata, req.headers);
    ...
    if (claudeRequirementChecklist && claudeRequirementChecklist.sourceHash !== priorClaudeChecklistHash) {
        recordSessionEvent(claudeSessionKey, claudeIdentity.userId, claudeIdentity.orgId, "requirements_checklist", "completion-gate", `Checklist initialized (must=${claudeRequirementChecklist.must.length}, should=${claudeRequirementChecklist.should.length})`, traceReqId);
    }
    ...
    if (claudeWorkspaceStatus === "pending" && pendingClaudeWorkspaceToolId) {
        const toolResult = extractClaudeToolResult(body.messages, pendingClaudeWorkspaceToolId);
        if (toolResult !== null) {
            const parsedCtx = parseWorkspaceContextOutput(toolResult);
            if (parsedCtx) {
    ...
    if (config.SYNESIS_YARN_EVIDENCE_PREFETCH_ENABLED && latestClaudeUser) {
        ...
        if (claudePrefetchText.length > 0) {
            claudePrefetchResult = await runEvidencePrefetch(claudePrefetchText, knowledgeSearch, config.SYNESIS_YARN_EVIDENCE_PREFETCH_TIMEOUT_MS, config.SYNESIS_YARN_EVIDENCE_CONFIDENCE_MIN, { retryEnabled: config.SYNESIS_YARN_EVIDENCE_PREFETCH_RETRY_ENABLED });
    ...
    if (config.SYNESIS_YARN_PATTERN_RECALL_ENABLED && latestClaudeUser && !claudePrefetchResult?.matched) {
        ...
        if (claudePatternText.length > 0) {
            claudePatternResult = await runPatternPrefetch(claudePatternText, knowledgeSearch, config.SYNESIS_YARN_EVIDENCE_PREFETCH_TIMEOUT_MS, claudeWorkingPhase);
        }
    }
    const claudeCombinedConfidence = Math.max(claudePrefetchResult?.confidence ?? 0, claudePatternResult?.confidence ?? 0);
    ...
    const claudeLastToolUseId = [...body.messages]
        .reverse()
    ...
    const claudeToolProgress = detectToolProgress(session, normalizedFromClaude.messages);
    ...
    if (!claudePolicyPrecheck.allow) {
        ...
        if (config.SYNESIS_YARN_TOOL_LOOP_SOFT_FAIL_ENABLED && claudePolicyPrecheck.softFailClass === "tool_loop") {
            ...
            session.toolLoopNoUserAckCount = 0;
            session.history.push({ role: "assistant", content });
            persistSessionAndUsage(session, traceReqId, claudeOrchestration.selectedModel, usage, Date.now() - started, "end_turn", 0);
            maybeCheckpoint(session);
            recordSessionEvent(claudeSessionKey, claudeIdentity.userId, claudeIdentity.orgId, "tool_loop_soft_fail", "deterministic-policy", claudePolicyPrecheck.rejectReason ?? "Tool loop soft fail", traceReqId);
    ...
    const claudeEnriched = enrichWithFrameAndManifest(normalizedFromClaude.messages, claudeSessionKey, effectiveClaudeAdapterBlock, claudePromptContext, { projectRoot: effectiveClaudePathCtx.projectRoot, shellCwd: effectiveClaudePathCtx.shellCwd });
    let enrichedClaudeMsgs = appendCriticBlock(claudeEnriched.messages, claudeRequirementChecklist);
    let claudeSensemakingResult;
    if (config.SYNESIS_YARN_SENSEMAKING_ENABLED) {
        const claudeGapCtx = {
    ...
    if (config.SYNESIS_YARN_JITTER_BUFFER_ENABLED) {
        ...
        enrichedClaudeMsgs = applyJitter(stableMessages, jitterBlock);
    }
    ...
    enrichedClaudeMsgs = claudeTrustResult.messages;
    const openAIShape = {
        model: claudeOrchestration.selectedModel,
        messages: enrichedClaudeMsgs,
        stream: body.stream,
    ...
    openAIShape.messages = injectSessionContext(openAIShape.messages, session);
    {
        ...
        if (claudeBlocks.length > 0) {
            ...
            openAIShape.messages = claudeMsgs;
        }
    }
    if (config.SYNESIS_YARN_ARTIFACT_RETRIEVAL_ENABLED) {
        openAIShape.tools = artifactRetrieval.injectToolOpenAI(openAIShape.tools);
    }
    if (config.SYNESIS_YARN_KNOWLEDGE_SEARCH_ENABLED) {
        openAIShape.tools = knowledgeSearch.injectToolOpenAI(openAIShape.tools);
    }
    ...
    const sdkTools = claudeToolsToSDK(effectiveClaudeTools);
    ...
    const claudeModelMessages = claudeModelToolPrompt
        ? [{ role: "system", content: claudeModelToolPrompt }, ...messages]
        : messages;
    ...
    if (body.stream) {
        ...
        if (!claudeAdmission.admitted) {
            ...
            recordSessionEvent(claudeSessionKey, claudeIdentity.userId, claudeIdentity.orgId, "stream_admission_reject", "stream-admission", claudeAdmission.reason ?? "stream admission rejected", traceReqId);
        ...
        if (!circuitBreakers.allowRequest(resolved.resolvedModelId, claudeIdentity.orgId)) {
            ...
            recordSessionEvent(claudeSessionKey, claudeIdentity.userId, claudeIdentity.orgId, "breaker_open_reject", "circuit-breaker", `Circuit breaker open for ${resolved.resolvedModelId} (claude stream)`, traceReqId, { model: resolved.resolvedModelId });
            reply.header("Retry-After", "30");
        ...
        const pendingClaudeTextDeltas = [];
        const pendingClaudeToolIds = new Set();
        const claudeToolBuffer = new Map();
        const resolvedTier = tierRegistry.getTierConfig(resolved.resolvedModelId);
        const isLocalLikeBaseUrl = !!resolvedTier?.baseUrl &&
        ...
        const flushClaudeTextBlock = (text) => {
            if (!text)
                return;
            safeSse(reply, "content_block_start", {
                type: "content_block_start",
        ...
        try {
            for await (const part of streamed.fullStream) {
                if (part.type === "text-delta") {
                    ...
                    if (pendingClaudeToolIds.has(tdId))
                        continue;
                    ...
                    const tcFull = part;
                    if (tcFull.toolName === ARTIFACT_TOOL_NAME || tcFull.toolName === KNOWLEDGE_TOOL_NAME)
                        continue;
                    const buf = claudeToolBuffer.get(tcFull.toolCallId ?? "");
                    let finalInput = (tcFull.input ?? {});
                    ...
                    if (claudeAdapter.remapToolArgs) {
                        ...
                        if (wasRemapped)
                            toolArgHardeningStats.remappedArgsCount += 1;
                    }
                    ...
                    if (repair) {
                        ...
                        app.log.warn({
                            ...
                            filePath: tcFull.input?.file_path ?? tcFull.input?.path,
                    ...
                    if (bashRepair) {
                        ...
                        requestToolRepairs += 1;
                        app.log.warn({ reqId: traceReqId, toolName: emitToolName, bashRepaired: bashRepair.repaired }, "bash_tool_args_repaired");
                    }
                    const governed = governToolCall({
                    ...
                    if (governed.constrainedToRoot) {
                        toolArgHardeningStats.projectRootConstrainedCount += 1;
                        app.log.info({ reqId: traceReqId, toolName: emitToolName, toolCallId: tcFull.toolCallId }, "file_tool_path_constrained_to_project_root");
                    }
                    if (governed.blockedBashDrift) {
                        toolArgHardeningStats.blockedBashPathDriftCount += 1;
                        app.log.warn({ reqId: traceReqId, toolName: emitToolName, toolCallId: tcFull.toolCallId }, "bash_path_drift_blocked");
                    }
                    if (governed.validationMissing.length > 0) {
            ...
            if (claudeAdapter.family === "qwen3-coder"
                && isLocalLikeBaseUrl
        ...
        if (stopReason !== "tool_use" && pendingClaudeTextDeltas.length > 0) {
            ...
            if (gate.applied) {
                recordSessionEvent(claudeSessionKey, claudeIdentity.userId, claudeIdentity.orgId, "completion_gap", "completion-gate", `Missing must-have requirements (${gate.missingMust})`, traceReqId);
            }
        ...
        try {
            usage = readUsage(await streamed.totalUsage);
        }
        catch { /* stream aborted */ }
        safeSse(reply, "message_delta", {
            type: "message_delta",
        ...
        try {
            claudeStreamedText = await streamed.text;
        }
        catch { /* stream aborted */ }
        ...
        const claudeStreamMsgCounts = countMessageRoles(openAIShape.messages);
        pushDiagnostic({
    ...
    if (!circuitBreakers.allowRequest(resolved.resolvedModelId, claudeIdentity.orgId)) {
        ...
        recordSessionEvent(claudeSessionKey, claudeIdentity.userId, claudeIdentity.orgId, "breaker_open_reject", "circuit-breaker", `Circuit breaker open for ${resolved.resolvedModelId} (claude)`, reqId, { model: resolved.resolvedModelId });
        reply.header("Retry-After", "30");
        return reply.code(503).send({ type: "error", error: { type: "overloaded_error", message: "Model provider temporarily unavailable. Try again shortly." } });
    }
    ...
    if (config.SYNESIS_YARN_KNOWLEDGE_SEARCH_ENABLED) {
        ...
        for (const kc of knowledgeCalls) {
            await knowledgeSearch.resolve(kc.input, {
                orgId: claudeIdentity.orgId,
                userId: claudeIdentity.userId,
            });
        }
    ...
    if (stopReason === "end_turn") {
        ...
        if (gate.applied) {
            recordSessionEvent(claudeSessionKey, claudeIdentity.userId, claudeIdentity.orgId, "completion_gap", "completion-gate", `Missing must-have requirements (${gate.missingMust})`, reqId);
        }
        else if (claudeRequirementChecklist) {
    ...
    const claudeNonStreamMsgCounts = countMessageRoles(openAIShape.messages);
    pushDiagnostic({
