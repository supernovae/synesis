...
function resolveInsideRoot(projectRoot, rel) {
...
async function walkDir(dir, maxDepth, includeHidden) {
    const out = [];
...
async function walkFilesForSearch(root, maxDepth) {
    const out = [];
        ...
        for (const e of entries) {
            ...
            if (e.isDirectory()) {
                if (item.depth < maxDepth)
                    queue.push({ abs, depth: item.depth + 1 });
            }
            else if (e.isFile()) {
                out.push(abs);
            }
...
export const getRuntimeContextTool = {
...
export const applyPatchTool = {
    ...
    async handler(input) {
        ...
        if (idx < 0)
            return { filePath: input.filePath, replaced: false };
    ...
    async handler(input) {
        ...
        if (out.exitCode !== 0 && out.stderr.includes("ENOENT")) {
            ...
            for (const f of files) {
                if (matches.length >= input.headLimit)
                    break;
...
function makeRunnerTool(name, description, presets) {
    return {
        name,
        description,
...
export const runTestTool = makeRunnerTool("run_test", "Run tests using allowlisted deterministic command presets.", RUN_TEST_PRESETS);
export const runBuildTool = makeRunnerTool("run_build", "Run build using allowlisted deterministic command presets.", RUN_BUILD_PRESETS);
export const runLintTool = makeRunnerTool("run_lint", "Run lint/check using allowlisted deterministic command presets.", RUN_LINT_PRESETS);
export const formatCodeTool = makeRunnerTool("format_code", "Run formatter using allowlisted deterministic command presets.", FORMAT_PRESETS);
...
export const gitAddGuardedTool = {
    ...
    async handler(input) {
        ...
        for (const f of input.files) {
            ...
            if (BlockedGitPaths.some((re) => re.test(f)))
                blocked.push(f);
            else
                safe.push(f);
        }
...
export const gitCommitGuardedTool = {
