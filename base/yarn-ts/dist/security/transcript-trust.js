 ...
 */
import { makeUntrusted, serializeStableJson, scanText, scanWebContent, sanitize, scanResultToPayload, emitSecurityEvent, TRUST_POLICY_COMPACT, } from "@synesis/context-trust";
...
function wrapInPacket(packet) {
    return serializeStableJson(packet);
}
export function applyTrustPackets(messages, config, ctx, ingestConfig, logger) {
    ...
    for (const msg of messages) {
        ...
        if (role === "user" || role === "tool") {
            const sourceType = role === "user" ? "user_message" : "tool_result";
            if (scanEnabled && raw) {
