...
export function analyzeGaps(ctx) {
    const known = [];
    const unknown = [];
    const knowBetter = [];
...
function classifyRecall(ctx, known, unknown, knowBetter) {
...
function classifyEvidence(ctx, known, unknown, knowBetter) {
...
function classifyVerification(ctx, known, unknown, knowBetter) {
    const { verificationState } = ctx;
    if (verificationState.round === 0)
        return;
...
function classifyLanguageCoverage(ctx, known, unknown, _knowBetter) {
...
export function shouldTriggerSensemaking(gaps, decision, consecutiveFailedVerifications, gapThreshold = DEFAULT_GAP_THRESHOLD) {
