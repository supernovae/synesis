...
export function assessComplexity(text, fileCount) {
    const signals = [];
    for (const re of TINY_PATTERNS) {
        if (re.test(text))
            signals.push(`tiny:${re.source.slice(0, 30)}`);
    }
    ...
    for (const re of LARGE_PATTERNS) {
        if (re.test(text))
            largeSignals.push(`large:${re.source.slice(0, 30)}`);
    }
    ...
    for (const re of MEDIUM_PATTERNS) {
        if (re.test(text))
            mediumSignals.push(`medium:${re.source.slice(0, 30)}`);
    }
    if (mediumSignals.length > 0 || (fileCount ?? 0) >= 5) {
        if (fileCount !== undefined && fileCount >= 5)
            mediumSignals.push("medium:file_count");
...
export function classify(text, fileCount) {
    return {
