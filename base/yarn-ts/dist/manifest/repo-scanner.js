...
export function scanForManifest(input) {
    ...
    if (bestKind === "go_cli" && kindVotes.has("go_http_service")) {
        ...
        if (httpVotes >= bestVotes)
            bestKind = "go_http_service";
    }
    const observedFiles = input.filePaths.slice(0, 50).map((p) => ({
        path: p,
        required: false,
        purpose: "",
        status: "present",
    ...
    const observedDirs = [...dirs].slice(0, 30).map((d) => ({
        path: d,
        required: false,
        purpose: "",
        status: "present",
