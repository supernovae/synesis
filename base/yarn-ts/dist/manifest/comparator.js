 ...
 */
export function compareManifests(targetRaw, observedRaw) {
    ...
    const missingFiles = tFiles.filter((f) => !observedPaths.has(normalizePath(f.path)));
    const extraFiles = [...observedPaths].filter((p) => !targetPaths.has(p));
    const observedDirPaths = new Set(oDirs.map((d) => normalizePath(d.path)));
    const missingDirectories = tDirs.filter((d) => !observedDirPaths.has(normalizePath(d.path)));
    const observedToolNames = new Set(oTools.map((t) => t.name.toLowerCase()));
    const missingTools = tTools.filter((t) => !observedToolNames.has(t.name.toLowerCase()));
    ...
    const missingCount = missingFiles.length + missingDirectories.length + missingTools.length;
    const structuralScore = totalTargetItems > 0
...
function findMissingDocSections(target, observed) {
    const missing = [];
    for (const pattern of target.documentationPatterns ?? []) {
        if (!pattern.required)
            continue;
        const obsPattern = (observed.documentationPatterns ?? []).find((p) => p.name.toLowerCase() === pattern.name.toLowerCase());
        if (!obsPattern) {
