export class DotnetReducer {
    family = "dotnet";
    reduce(input) {
        const raw = input.raw;
        const lines = raw.split("\n");
        let buildOk = null;
        if (/Build succeeded/i.test(raw))
            buildOk = true;
        if (/Build FAILED/i.test(raw))
            buildOk = false;
        const errLines = [];
        const warnLines = [];
        ...
        if (failedM)
            failedTests = parseInt(failedM[1], 10);
        ...
        const looksDotnet = buildOk !== null ||
        ...
        if (failedTests > 0)
            parts.push(`tests failed: ${failedTests}`);
        if (errLines.length > 0) {
            parts.push("errors:");
            errLines.slice(0, limit).forEach((e, i) => parts.push(`  ${i + 1}. ${e}`));
            if (errLines.length > limit)
                parts.push(`  ... ${errLines.length - limit} more`);
        }
        if (warnLines.length > 0) {
            parts.push("warnings:");
            warnLines.slice(0, limit).forEach((w, i) => parts.push(`  ${i + 1}. ${w}`));
            if (warnLines.length > limit)
                parts.push(`  ... ${warnLines.length - limit} more`);
        }
