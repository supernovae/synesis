export class RspecReducer {
    ...
    reduce(input) {
        ...
        const looksRspec = summaryMatch !== null ||
            /^rspec\s+\.\//m.test(raw) ||
            /Failure\/Error:/i.test(raw);
        if (!looksRspec)
            return null;
        if (examples === 0 && failureCount === 0 && summaryMatch === null) {
            if (failedExamples.length === 0 && errorMsgs.length === 0)
                return null;
        ...
        if (failedExamples.length > 0) {
            ...
            if (failedExamples.length > limit)
                parts.push(`  ... ${failedExamples.length - limit} more`);
        }
        if (errorMsgs.length > 0) {
            parts.push("errors:");
            errorMsgs.slice(0, limit).forEach((e, i) => parts.push(`  ${i + 1}. ${e}`));
            if (errorMsgs.length > limit)
                parts.push(`  ... ${errorMsgs.length - limit} more`);
        }
