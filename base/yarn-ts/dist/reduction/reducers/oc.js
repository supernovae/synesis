    ...
    reduce(input) {
        ...
        for (const line of lines) {
            ...
            if (/^Name:\s+/.test(line))
                ...
                if (inEvents && !/^Events:/.test(t))
                    inEvents = false;
                if (inConditions && !/^\s+(True|False|Unknown)\s+/.test(line))
                    inConditions = false;
            }
        ...
        if (isEventsTable) {
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                if (!row || /^LAST SEEN/i.test(row))
                    continue;
                const m = row.match(/\b(Normal|Warning)\b/i);
                if (m)
                    types.set(m[1], (types.get(m[1]) ?? 0) + 1);
            }
            if (types.size === 0 && lines.length < 3)
                return null;
            const parts = [`<TOOL_REDUCED family="oc" resources="${lines.length - 1}">`, "events summary:"];
        ...
        if (/^In project\s+/i.test(input.raw) || /on server\s+https:\/\//i.test(input.raw)) {
            ...
            if (interesting.length > limit)
                parts.push(`  ... ${interesting.length - limit} more`);
            parts.push("</TOOL_REDUCED>");
        ...
        if (!hasDescribe && resources.length === 0)
            return null;
        const limit = input.context.profile === "ultra" ? 8 : 16;
        const parts = [`<TOOL_REDUCED family="oc" resources="${Math.max(resources.length, 1)}">`];
        if (nameLine)
            parts.push(`  ${nameLine}`);
        if (namespaceLine)
            parts.push(`  ${namespaceLine}`);
        if (statusLine)
        ...
        if (resources.length > limit)
            parts.push(`  ... ${resources.length - limit} more`);
        if (events.length > 0) {
