...
export class TerraformReducer {
    ...
    reduce(input) {
        ...
        if (changes.length === 0 && errors.length === 0 && !planSummary && !applySummary)
            return null;
        const limit = input.context.profile === "ultra" ? 8 : 16;
        const { items: enriched, enrichedLines, bypassEligible } = enrichItems(this.family, errorItems.slice(0, 5));
        const parts = [`<TOOL_REDUCED family="terraform" changes="${changes.length}">`];
        if (planSummary)
        ...
        if (changes.length > 0) {
            ...
            if (changes.length > limit)
                parts.push(`  ... ${changes.length - limit} more changes`);
        }
