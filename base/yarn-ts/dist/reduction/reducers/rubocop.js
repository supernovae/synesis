...
export class RubocopReducer {
    family = "rubocop";
    reduce(input) {
        const lines = input.raw.split("\n");
        const offenses = [];
        const items = [];
        let filesInspected = null;
        let offensesDetected = null;
        ...
        const looksRubocop = input.raw.includes("Offenses:") ||
            input.raw.includes("files inspected") ||
            /\.rb:\d+:\d+:\s*[CWEF]:\s*[\w/]+:/m.test(input.raw);
        if (!looksRubocop && offenses.length === 0)
            return null;
        if (offenses.length === 0 && filesInspected === null && offensesDetected === null)
            return null;
        ...
        if (enrichedLines.length > 0) {
            ...
            if (offenses.length > limit)
                parts.push(`  ... ${offenses.length - limit} more`);
        }
