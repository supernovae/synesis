...
function walkGcloud(val, acc) {
    ...
    if (typeof val === "object") {
        ...
        if (typeof o.status === "string")
            acc.statuses.push(o.status);
        if (typeof o.zone === "string")
            acc.zones.push(o.zone.split("/").pop() ?? o.zone);
        if (typeof o.projectId === "string")
            acc.projects.push(o.projectId);
        for (const k of Object.keys(o))
            walkGcloud(o[k], acc);
    }
}
function parseGcloudTable(lines) {
    let header = "";
    const rows = [];
    for (const line of lines) {
        const t = line.trim();
        if (!t || t.startsWith("Listed") || t.startsWith("Using"))
            continue;
...
export class GcloudReducer {
    ...
    reduce(input) {
        ...
        const parts = [`<TOOL_REDUCED family="gcloud" resources="${resourceCount}">`];
