    ...
    reduce(input) {
        ...
        for (const line of lines) {
            ...
            if (/^[d-][rwx-]{9}/.test(trimmed) || trimmed.startsWith("total ")) {
                ...
                if (lastSlash > 0)
                    dirs.add(trimmed.slice(0, lastSlash));
                files.push(trimmed);
            }
            else {
                files.push(trimmed);
        ...
        const parts = [`<TOOL_REDUCED family="ls-tree" entries="${totalEntries}" dirs="${dirs.size}" files="${files.length}">`];
        if (treeLines.length > 0) {
            treeLines.slice(0, limit).forEach((l) => parts.push(l));
            if (treeLines.length > limit)
                parts.push(`... ${treeLines.length - limit} more entries`);
        }
        else {
            if (dirs.size > 0)
                parts.push(`dirs: ${[...dirs].slice(0, 10).join(", ")}${dirs.size > 10 ? ` (+${dirs.size - 10})` : ""}`);
            const topFiles = files.slice(0, limit);
            topFiles.forEach((f) => parts.push(`  ${f}`));
            if (files.length > limit)
                parts.push(`  ... ${files.length - limit} more files`);
        }
