export class WebpackReducer {
    ...
    reduce(input) {
        ...
        for (const line of lines) {
            ...
            else if (/^(asset|Entrypoint)/i.test(t) &&
        ...
        const looksWebpack = /webpack|compiled\s+with|Entrypoint|asset\s+[\w.-]+\.(js|css|mjs)/i.test(raw) ||
        ...
        if (moduleCount !== null)
            parts.push(`modules: ${moduleCount}`);
        if (buildTime)
        ...
        if (assets.length > 0) {
            ...
            if (assets.length > limit)
                parts.push(`  ... ${assets.length - limit} more`);
        }
        if (errors.length > 0) {
            parts.push("errors:");
