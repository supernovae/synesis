export class KubectlReducer {
    ...
    reduce(input) {
        ...
        if (resources.length === 0 && events.length === 0 && conditions.length === 0)
            return null;
        const limit = input.context.profile === "ultra" ? 10 : 20;
        const parts = [`<TOOL_REDUCED family="kubectl" resources="${resources.length}" events="${events.length}">`];
