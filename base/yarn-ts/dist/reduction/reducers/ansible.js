export class AnsibleReducer {
    ...
    reduce(input) {
        ...
        if (fails.length === 0 && changed.length === 0 && !recapLine)
            return null;
        const parts = [`<TOOL_REDUCED family="ansible">`];
        if (playName)
            parts.push(playName);
        if (recapLine && recapLine !== "PLAY RECAP")
            parts.push(recapLine);
        if (fails.length > 0) {
            parts.push(`failures (${fails.length}):`);
            fails.slice(0, 5).forEach((f, i) => parts.push(`  ${i + 1}. ${f}`));
        }
        if (changed.length > 0)
            parts.push(`changed: ${changed.length} tasks`);
