function parseOperationsTotal(segment) {
    let total = 0;
    const inst = /(\d+)\s+installs?/i.exec(segment);
    if (inst)
        total += parseInt(inst[1], 10);
    const upd = /(\d+)\s+updates?/i.exec(segment);
    if (upd)
        total += parseInt(upd[1], 10);
    const rem = /(\d+)\s+removals?/i.exec(segment);
    if (rem)
        total += parseInt(rem[1], 10);
    return total;
}
export class ComposerReducer {
    family = "composer";
    reduce(input) {
        const raw = input.raw;
        const lines = raw.split("\n");
        const looksComposer = /^Installing\s+/m.test(raw) ||
        ...
        if (/Writing lock file/i.test(raw))
            metaLines.push("Writing lock file");
        if (/Generating autoload files/i.test(raw))
            metaLines.push("Generating autoload files");
        let declaredTotal = 0;
        ...
        if (opLines.length > 0) {
            ...
            if (opLines.length > limit)
                parts.push(`  ... ${opLines.length - limit} more`);
        }
