export class StracePerfReducer {
    family = "strace-perf";
    reduce(input) {
        ...
        if (syscalls.size === 0 && summaryLines.length === 0)
            return null;
        const topSyscalls = [...syscalls.entries()].sort((a, b) => b[1] - a[1]).slice(0, 10);
        const parts = [`<TOOL_REDUCED family="strace-perf" unique_syscalls="${syscalls.size}" errors="${errors.length}">`];
