export class DockerBuildReducer {
    family = "docker-build";
    reduce(input) {
        ...
        if (stepCount === 0 && errors.length === 0 && !finalImage)
            return null;
        const parts = [`<TOOL_REDUCED family="docker-build" steps="${stepCount}" cached="${cachedCount}">`];
        if (finalImage)
            parts.push(`result: ${finalImage}`);
        if (errors.length > 0) {
