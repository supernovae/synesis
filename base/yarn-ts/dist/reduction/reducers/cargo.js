...
export class CargoReducer {
    ...
    reduce(input) {
        ...
        for (const line of lines) {
            ...
            if (ec) {
                ...
                if (!trimmed.includes("generated ") && !trimmed.includes(" warning generated")) {
                    ...
                    if (wm)
                        items.push({ message: wm[2], ruleId: wm[1] ?? undefined });
                }
            }
                ...
                if (lastErr)
                    errors[errors.length - 1] = `${lastErr} ${trimmed}`;
                const lastItem = items[items.length - 1];
                if (lastItem) {
                    const fileParts = /^-->\s+(.+?):(\d+):(\d+)/.exec(trimmed);
                    if (fileParts)
                        lastItem.file = fileParts[1];
                }
            }
        }
        if (errors.length === 0 && warnings.length === 0 && !testSummary)
            return null;
        ...
        const parts = [`<TOOL_REDUCED family="cargo" compiled="${compileCount}">`];
        if (testSummary)
