...
export class ClippyReducer {
    ...
    reduce(input) {
        ...
        const flushPendingAt = (locStr, file) => {
            if (pendingW) {
        ...
        for (const line of lines) {
            ...
            if (wg)
                summaryWarnings = parseInt(wg[1], 10);
        ...
        if (summaryWarnings !== null)
            parts.push(`rustc summary: ${summaryWarnings} warnings generated`);
        if (enrichedLines.length > 0) {
            parts.push(...enrichedLines);
            if (rawItems.length > limit)
                parts.push(`  ... ${rawItems.length - limit} more`);
        }
