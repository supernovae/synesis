    ...
    reduce(input) {
        ...
        for (const line of lines) {
            ...
            if (!trimmed)
                continue;
            let level = "info";
            if (/\b(ERROR|FATAL|CRITICAL|SEVERE)\b/i.test(trimmed))
                level = "error";
            ...
            if (level === "error" && errorLines.length < 8)
                errorLines.push(trimmed.slice(0, 200));
            if (level === "warn" && warnLines.length < 5)
                warnLines.push(trimmed.slice(0, 200));
        }
        const parts = [`<TOOL_REDUCED family="log-stream" lines="${lines.length}">`];
