export class PipInstallReducer {
    family = "pip-install";
    reduce(input) {
        ...
        if (errors.length === 0 && warnings.length === 0 && installedCount === 0 && alreadySatisfied === 0)
            return null;
        const parts = [`<TOOL_REDUCED family="pip-install">`];
        if (installedCount > 0)
            parts.push(`installed ${installedCount} packages`);
        if (alreadySatisfied > 0)
            parts.push(`${alreadySatisfied} already satisfied`);
        if (errors.length > 0) {
