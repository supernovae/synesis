export class PhpunitReducer {
    ...
    reduce(input) {
        ...
        const looksPhpunit = /PHPUnit\s+[\d.]+/i.test(raw) ||
            /FAILURES!/i.test(raw) ||
        ...
        if (assertions > 0)
            parts.push(`assertions: ${assertions}`);
        if (failures > 0)
        ...
        if (failedBlocks.length > 0) {
            ...
            if (failedBlocks.length > limit * 2)
                parts.push(`  ... ${failedBlocks.length - limit * 2} more`);
        }
