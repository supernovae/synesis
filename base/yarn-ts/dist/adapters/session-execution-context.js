...
function headerOne(headers, name) {
    ...
    if (Array.isArray(v))
        return typeof v[0] === "string" ? v[0].trim() : undefined;
 ...
 */
export function parseSessionExecutionContext(headers, metadata) {
    ...
    if (rt && typeof rt === "object" && rt !== null) {
        const o = rt;
        if (typeof o.platform === "string" && o.platform.trim())
            platform = o.platform.trim();
        if (typeof o.os_version === "string" && o.os_version.trim())
            osVersion = o.os_version.trim();
 ...
 */
export function toSessionExecutionContextSystemBlock(ctx) {
    if (!ctx.projectRoot && !ctx.shellCwd && !hasAnyOptional(ctx)) {
        return "";
    }
    const lines = ["<SESSION_EXECUTION_CONTEXT>"];
    if (ctx.projectRoot) {
        lines.push(`project_root=${ctx.projectRoot}`);
        lines.push("Treat project_root as the repository/workspace anchor. Create new work under it; do not nest multiple directories with the same name (e.g. avoid proj/proj/proj).");
        lines.push("If the workspace is empty, add files at the root (e.g. go.mod, main.go) instead of mkdir && cd into repeated path segments.");
        lines.push("Keep Read/Write/Edit paths relative to project_root. Shell cd only affects Bash; file tools stay workspace-relative to project_root.");
    }
    if (ctx.shellCwd) {
        lines.push(`shell_cwd=${ctx.shellCwd}`);
        lines.push("shell_cwd is the current task directory on the client when provided; prefer editing within the existing tree under project_root when both are set.");
        if (!ctx.projectRoot) {
            lines.push("Without project_root: do not mkdir && cd into a subfolder that repeats the last path segment of shell_cwd (e.g. avoid aws-cost-calculator/aws-cost-calculator). Add go.mod, main.go, etc. at shell_cwd when it is already the project root.");
        }
    }
    if (ctx.platform)
        lines.push(`platform=${ctx.platform}`);
    if (ctx.osVersion)
        lines.push(`os_version=${ctx.osVersion}`);
    if (ctx.shell)
        lines.push(`shell=${ctx.shell}`);
    if (ctx.gitSummary)
        lines.push(`git_summary=${ctx.gitSummary}`);
    if (ctx.clientModelLabel)
        lines.push(`client_model_label=${ctx.clientModelLabel}`);
    if (ctx.knowledgeCutoff)
        lines.push(`knowledge_cutoff=${ctx.knowledgeCutoff}`);
 ...
 */
function pathHygieneFallbackBlock() {
    return [
        "<PATH_HYGIENE>",
...
function shouldAppendPathHygieneFallback(coderClientHint) {
 ...
 */
export function appendPathContextToAdapterBlock(adapterBlock, headers, metadata, coderClientHint) {
...
export function resolveWorkspaceRootForCollapse(headers, metadata) {
