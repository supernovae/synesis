...
export class UsageWriter {
    ...
    enqueueUsageInsert(event) {
        this.enqueue({ type: "usage", event });
    }
    enqueueSafetyEventInsert(event) {
        this.enqueue({ type: "safety", event });
    }
    enqueueSessionEvent(event) {
        this.enqueue({ type: "session_event", event });
    }
    enqueueContinuityUpsert(userId, orgId, sessionKey, continuity) {
        this.enqueue({ type: "continuity", data: { userId, orgId, sessionKey, continuity } });
    }
    async ensureContinuityTable() {
        if (!this.pool || this._tablesEnsured)
            return;
        try {
    ...
    async loadLatestContinuity(userId, maxAgeMs = 7 * 24 * 60 * 60 * 1000) {
        if (!this.pool)
            return null;
        this._memoryStats.recallLoads++;
        try {
            const cutoff = new Date(Date.now() - maxAgeMs);
            const result = await this.pool.query(`SELECT current_task, key_findings, decisions, recent_files, updated_at
         FROM yarn_session_continuity
         WHERE user_id = $1 AND updated_at >= $2
         ORDER BY updated_at DESC
         LIMIT 1`, [userId, cutoff.toISOString()]);
    ...
    async upsertContinuity(data) {
        if (!this.pool)
            return;
        await this.pool.query(`INSERT INTO yarn_session_continuity (
    ...
    async flush() {
        if (!this.pool || this.flushing || this.queue.length === 0)
            return;
        this.flushing = true;
        try {
