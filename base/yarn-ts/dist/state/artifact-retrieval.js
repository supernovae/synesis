...
const ARTIFACT_DESCRIPTION = "Retrieve the full, uncompressed content of a tool result or validation output " +
...
const ARTIFACT_PARAMETERS = {
    ...
    properties: {
        ...
        query: {
            type: "string",
            description: "Optional keyword query to return only matching lines from the artifact. " +
...
export class ArtifactRetrievalService {
    ...
    injectToolOpenAI(tools) {
        if (!this.hasArtifacts())
            return tools;
        if (!tools)
            return [ARTIFACT_TOOL_SCHEMA_OPENAI];
        const exists = tools.some((t) => t.function?.name === ARTIFACT_TOOL_NAME);
        if (exists)
            return tools;
    ...
    injectToolClaude(tools) {
        if (!this.hasArtifacts())
            return tools;
        if (!tools)
            return [ARTIFACT_TOOL_SCHEMA_CLAUDE];
        const exists = tools.some((t) => t.name === ARTIFACT_TOOL_NAME);
        if (exists)
            return tools;
