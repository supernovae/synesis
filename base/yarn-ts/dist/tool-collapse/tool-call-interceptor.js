...
import { executeCollapsePlan } from "./tool-call-executor.js";
...
function fallbackPassthroughPlan(calls) {
    return {
        operations: calls.map((c) => ({ kind: "passthrough", calls: [c] })),
        log: [{ phase: "collapse", detail: "fallback_passthrough_all", atMs: Date.now() }],
    };
}
function countSdkCalls(plan) {
    let n = 0;
 ...
 */
export function planToSyntheticToolCalls(plan) {
    const out = [];
    for (const op of plan.operations) {
        if (op.kind === "passthrough") {
            for (const c of op.calls) {
                const input = typeof c.input === "object" && c.input !== null && !Array.isArray(c.input)
                    ? c.input
                    : { _raw: c.input };
...
export class ToolCallInterceptor {
     ...
     */
    async processImmediate(calls) {
        const ctx = this.buildContext();
        const incomingCount = calls.length;
        let dedupeStats;
        let plan = this.opts.dedupeLayer != null
            ? (() => {
        ...
        let validated = validateCollapsePlan(plan, ctx);
        ...
        const compactJson = executions.length > 0
