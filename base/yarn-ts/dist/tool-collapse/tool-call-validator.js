...
import { z } from "zod";
import { SYNESIS_BATCH_READ, SYNESIS_BATCH_SEARCH, SYNESIS_MERGE_PATCH, SYNESIS_REPO_CONTEXT, SYNESIS_RUN_TESTS, } from "./types.js";
...
export function resolveSafePath(workspaceRoot, userPath) {
...
export function isShellCommandAllowed(command, allowlist) {
    const c = command.trim();
    if (!c)
        return false;
    return allowlist.some((re) => re.test(c));
}
function validateBatchRead(op, ctx, opIndex) {
    const issues = [];
    ...
    for (const p of op.paths) {
        ...
        if (!r.ok)
            issues.push({ opIndex, message: `batch_read: ${r.error}: ${p}`, severity: "error" });
    }
    return issues;
}
function validateBatchSearch(op, ctx, opIndex) {
    const issues = [];
    if (!ctx.workspaceRoot) {
        return issues;
    }
    for (const it of op.items) {
        if (it.path) {
            ...
            if (!r.ok)
                issues.push({ opIndex, message: `batch_search: ${r.error}: ${it.path}`, severity: "error" });
        }
    }
    return issues;
}
function validateRepoContext(op, ctx, opIndex) {
    const issues = [];
    if (!ctx.workspaceRoot) {
    ...
    if (op.search.path) {
        ...
        if (!r.ok)
            issues.push({ opIndex, message: `repo_context search path: ${r.error}`, severity: "error" });
    }
    for (const rd of op.reads) {
        const r = resolveSafePath(ctx.workspaceRoot, rd.path);
        if (!r.ok)
            issues.push({ opIndex, message: `repo_context read: ${r.error}: ${rd.path}`, severity: "error" });
    }
    return issues;
}
function validateMergePatch(op, ctx, opIndex) {
    const issues = [];
    if (!ctx.workspaceRoot) {
    ...
    for (const f of op.files) {
        ...
        if (!r.ok)
            issues.push({ opIndex, message: `merge_patch: ${r.error}: ${f.path}`, severity: "error" });
        const patch = f.patch;
...
function validateRunTests(op, ctx, opIndex) {
    const issues = [];
...
export function validateCollapsePlan(plan, ctx) {
    const issues = [];
...
export function parseSyntheticToolArgs(name, args) {
    try {
...
export function defaultShellAllowlistFromEnv(pattern) {
    const parts = pattern
        .split("|")
        .map((s) => s.trim())
        .filter(Boolean);
