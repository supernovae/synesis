...
function collectFilteredTokens(text) {
    const words = text
        .toLowerCase()
        .replace(/[^a-z0-9\s]/g, " ")
        .split(/\s+/)
...
function priorityForClause(raw) {
    if (/\b(optional|nice to have|if possible|could)\b/i.test(raw))
        return "should";
    if (/\b(create|build|present|display|accordingly|formatted|cli)\b/i.test(raw))
        return "should";
    return "must";
}
export function buildChecklistFromPrompt(prompt, sourceHash) {
...
export function evaluateRequirementCoverage(checklist, evidenceText) {
    const text = evidenceText.toLowerCase().replace(/\s+/g, " ").trim();
    const evidenceTokens = tokenizeEvidence(text);
    const matched = new Set();
    const covered = (item) => {
        if (item.coverageTerms.length === 0)
            return false;
        let hits = 0;
        for (const term of item.coverageTerms) {
            if (evidenceTokens.has(canonicalToken(term)))
                hits += 1;
        }
...
export function summarizeMissingCoverage(report) {
