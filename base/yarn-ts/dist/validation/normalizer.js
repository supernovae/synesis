...
function detectFamily(toolName, raw) {
    ...
    if (t.includes("tsc") || raw.includes("error TS"))
        return "typescript";
    if (t.includes("eslint") || raw.includes("eslint"))
        return "eslint";
    if (t.includes("ruff"))
        return "ruff";
    if (t.includes("mypy"))
        return "mypy";
    if (t.includes("pylint"))
        return "pylint";
    if (t.includes("jest"))
        return "jest";
    if (t.includes("cargo") || t.includes("clippy") || t.includes("rustc"))
        return "cargo";
    if (t.includes("golangci") || t.includes("golint"))
        return "golangci-lint";
    if (t.includes("terraform") || t.includes("tf_validate") || t.includes("tofu"))
        return "terraform";
    if (t.includes("tfsec"))
        return "tfsec";
    if (t.includes("trivy"))
        return "trivy";
    if (t.includes("semgrep"))
        return "semgrep";
    if (t.includes("pytest") || raw.includes("FAILED") || raw.includes("E       assert"))
        return "pytest";
    if (t.includes("go test") || t.includes("go_test"))
        return "go";
    if (raw.includes("Error:") && raw.includes(" on ") && raw.includes(" line "))
        return "terraform";
    return "generic";
}
...
function parseLineFindings(family, raw, maxFindings, maxExcerptChars) {
    if (family === "terraform") {
...
function parseTerraformText(raw, maxFindings, maxExcerptChars) {
    const blocks = raw
...
/* ── Summary builder ───────────────────────────────────────────── */
function buildSummary(family, findings, format) {
    const summaryLines = [];
    for (let i = 0; i < findings.length; i++) {
        ...
        if (f.likelyRootCause)
            summaryLines.push(`   Root cause: ${f.likelyRootCause}`);
        if (f.suggestedNextAction)
            summaryLines.push(`   Action: ${f.suggestedNextAction}`);
    }
    return [
 ...
 */
export function normalizeValidationOutput(input) {
...
function shouldTryTierCFallback(input, envelope) {
    if (!input.rawOutput.trim())
        return false;
    if (envelope.family !== "generic")
        return false;
    if (envelope.findings.length !== 1)
        return false;
    const f = envelope.findings[0];
    // Tier C is only useful when deterministic extraction failed to get structure.
    if (f.file || f.line || f.column || f.ruleId)
        return false;
    return true;
}
export async function normalizeValidationOutputWithTierC(input, options) {
    const base = normalizeValidationOutput(input);
    if (!options.enabled || !options.fallback)
        return base;
    if (!shouldTryTierCFallback(input, base))
        return base;
    try {
