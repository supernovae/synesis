 ...
 */
export function looksLikeClarificationTurnAssistantMessage(text) {
    const t = text.trim();
    if (t.length < 48)
        return false;
    ...
    if (ratio >= 0.28)
        return true;
    if (/\bclarifying questions\b/i.test(t))
        return true;
    if (/\bbefore I (?:start|implement|begin|write|code|create|build)\b/i.test(t))
        return true;
    if (/\b(?:a few|some) questions\b/i.test(t) && linesWithQuestion >= 1)
        return true;
    const numberedQuestionBlocks = (t.match(/(?:^|\n)\s*(?:\d+[\.\)]|[*•-])\s+[^\n]*\?/g) ?? []).length;
    if (numberedQuestionBlocks >= 2)
        return true;
    return false;
}
