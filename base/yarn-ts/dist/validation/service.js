...
import { normalizeValidationOutput, normalizeValidationOutputWithTierC, } from "./normalizer.js";
...
export class ValidationNormalizationService {
    ...
    constructor(config, artifactStore = new ArtifactStore()) {
        this.config = config;
    ...
    normalizeMessages(messages) {
        ...
        const out = messages.map((m) => {
            if (typeof m.content !== "string")
                return m;
            if (!this.shouldNormalize(m.name, m.content))
                return m;
            ...
            const decision = applyAdmissionPolicy(envelope, m.content, {
    ...
    async normalizeMessagesAsync(messages, tierCFallback) {
        let normalizedCount = 0;
        const out = [];
    ...
    shouldNormalize(toolName, content) {
        const name = (toolName ?? "").toLowerCase();
        if (VALIDATION_TOOL_HINTS.some((h) => name.includes(h)))
            return true;
        if (content.length >= this.config.SYNESIS_YARN_VALIDATION_MAX_RAW_CHARS)
            return true;
        ...
        if (trimmed.includes('"$schema"') && trimmed.includes("sarif"))
            return true;
        if (trimmed.includes('"runs"') && trimmed.includes('"results"'))
            return true;
        ...
        if (lower.includes("error:") && lower.includes(" on ") && lower.includes(" line "))
            return true;
        return false;
    }
}
