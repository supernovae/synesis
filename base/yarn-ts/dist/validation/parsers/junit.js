...
export function isJunit(raw) {
    const trimmed = raw.trimStart();
    return (trimmed.startsWith("<?xml") && (trimmed.includes("<testsuite") || trimmed.includes("<testsuites"))) || trimmed.startsWith("<testsuite") || trimmed.startsWith("<testsuites");
}
export function parseJunit(raw, fallbackFamily, maxFindings) {
    const findings = [];
    ...
    while ((match = TESTCASE_RE.exec(raw)) !== null) {
        ...
        if (!issueMatch)
            continue;
        const severity = errorMatch && !failureMatch ? "error" : "error";
...
function extractFileFromClassname(classname) {
    if (!classname)
        return undefined;
 ...
 */
function extractLineFromTrace(trace) {
