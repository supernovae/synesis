...
export class SessionContinuityService {
    ...
    extract(history) {
        ...
        const findings = uniq(allSentences
            .filter((s) => FINDING_PHRASES.test(s))
            .slice(-MAX_ITEMS));
        const decisions = uniq(allSentences
            .filter((s) => DECISION_PHRASES.test(s))
            .slice(-MAX_ITEMS));
        const recentFiles = uniq((allText.match(FILE_RE) ?? []).map((f) => f.trim())).slice(-MAX_ITEMS);
     ...
     */
    toSystemBlock(continuity) {
        const parts = ["<SESSION_CONTINUITY>"];
     ...
     * Returns null if the continuity has no useful content.
     */
    toRecallBlock(continuity) {
        const parts = ["<SESSION_RECALL source=\"prior_session\">"];
        if (continuity.currentTask) {
