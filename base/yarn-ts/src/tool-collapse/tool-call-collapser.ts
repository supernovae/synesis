...
export function extractReadPath(input: unknown): string | null {
...
export function extractSearchQuery(input: unknown): { query: string; path?: string } | null {
  const o = asRecord(input);
  const q =
...
export function extractPatch(input: unknown): { path: string; patch: string } | null {
  const o = asRecord(input);
  const path =
    (typeof o.path === "string" && o.path) ||
    (typeof o.file === "string" && o.file) ||
...
function logEntry(phase: CollapseLogEntry["phase"], detail: string, extra?: Partial<CollapseLogEntry>): CollapseLogEntry {
 ...
 */
function buildBatchRead(chunk: ParsedToolCall[]): BatchReadCollapsed {
  const pathToPrimaryId = new Map<string, string>();
  const pathToAllIds = new Map<string, string[]>();
  const order: string[] = [];
...
export interface ReadSearchDedupePrepassResult {
  calls: ParsedToolCall[];
  droppedReadIds: string[];
  droppedSearchIds: string[];
}
 ...
 */
export function dedupeReadsAndSearchesWithinSegments(calls: ParsedToolCall[]): ReadSearchDedupePrepassResult {
  const droppedReadIds: string[] = [];
  const droppedSearchIds: string[] = [];
  const out: ParsedToolCall[] = [];

  const flushSegment = (segment: ParsedToolCall[]) => {
      ...
      if (k === "read_file") {
        ...
        if (p) {
          if (seenPaths.has(p)) {
            ...
            const immediatelyAfterSamePathRead =
              prev !== null &&
      ...
      if (k === "search") {
        ...
        if (sq) {
          ...
          if (seenSearch.has(key)) {
            const prev = idx > 0 ? segment[idx - 1] : null;
            const immediatelyAfterSameSearch =
              prev !== null &&
              !NEVER_COLLAPSE_NAMES.has(prev.toolName) &&
...
function mergePatchGroup(chunk: ParsedToolCall[]): MergePatchCollapsed {
  const byFile = new Map<string, { patch: string; ids: string[] }>();
  for (const c of chunk) {
 ...
 */
export function collapseToolCallsLinear(slim: ParsedToolCall[], log: CollapseLogEntry[], originalIncomingCount: number): CollapsePlan {
  const operations: CollapsedOperation[] = [];
  log.push(
    logEntry("receive", `incoming ${slim.length} tool call(s) (${originalIncomingCount} original before dedupe pipeline)`),
  );
  ...
  while (i < slim.length) {
    ...
    if (k === "search") {
      ...
      while (i < slim.length) {
        ...
        if (classifyTool(x.toolName) !== "search") break;
        chunk.push(x);
        i += 1;
      }
      if (chunk.length >= 2) {
    ...
    if (k === "read_file") {
      ...
      while (i < slim.length) {
        const x = slim[i];
        if (NEVER_COLLAPSE_NAMES.has(x.toolName)) break;
        if (classifyTool(x.toolName) !== "read_file") break;
        chunk.push(x);
        i += 1;
      }
    ...
    if (k === "run_tests") {
      ...
      while (j < slim.length) {
        const x = slim[j];
        if (NEVER_COLLAPSE_NAMES.has(x.toolName)) break;
        if (classifyTool(x.toolName) !== "run_tests") break;
 ...
 */
export function collapseToolCalls(calls: ParsedToolCall[]): CollapsePlan {
  ...
  if (nDrop > 0) {
    log.push(
      logEntry("collapse", `prepass_segment_dedupe: dropped ${prepass.droppedReadIds.length} read(s), ${prepass.droppedSearchIds.length} search(es)`, {
