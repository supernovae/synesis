 ...
 */
export function deterministicTruncateMiddle(text: string, maxChars: number): string {
  if (maxChars < 80) return text.length <= maxChars ? text : `${text.slice(0, maxChars - 20)}…synesis_trunc`;
