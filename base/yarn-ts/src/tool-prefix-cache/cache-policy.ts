import type { CachePolicyInteraction, CollapsedKind } from "./types.js";
 ...
 */
const COLLAPSED_KIND_POLICY: Record<CollapsedKind, CachePolicyInteraction> = {
...
export function classifyCollapsedKindPolicy(kind: CollapsedKind): CachePolicyInteraction {
  return COLLAPSED_KIND_POLICY[kind] ?? "never";
}

/** Never cache raw patch text or speculative patch blobs from the model. */
export function mayCachePatchPayload(): false {
  return false;
}

/** Side-effectful or unknown passthrough tools — not cached here. */
export function isPassthroughCacheable(): false {
  return false;
}
