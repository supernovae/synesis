...
function stableItemsKey(items: Array<{ query: string; path?: string }>): string {
...
export class ToolPrefixCache {
  ...
  private readonly opts: ToolPrefixCacheOptions;
  private readonly genByNs = new Map<string, number>();
  private stats: ToolPrefixCacheStats = {
    readHits: 0,
    readMisses: 0,
    searchHits: 0,
    searchMisses: 0,
  ...
  constructor(opts: ToolPrefixCacheOptions) {
    this.opts = opts;
    this.store = new PrefixCacheStore(opts.maxEntries);
  }

  getStats(): ToolPrefixCacheStats {
    return { ...this.stats };
  }
  ...
  wrapExecutor(inner: ToolCollapseExecutor, workspaceRoot: string | null): ToolCollapseExecutor {
    ...
    const generation = (): number => this.genByNs.get(ns) ?? 0;

    const searchKey = (items: Array<{ query: string; path?: string }>): string => {
      return `v1:s:${ns}:${generation()}:${shaShort(stableItemsKey(items), 24)}`;
    };

    const repoKey = (search: { query: string; path?: string }, readPaths: string[]): string => {
    ...
    const allowStoreString = (s: string): boolean => {
    ...
    const allowStorePayload = (payload: unknown): boolean => {
    ...
    return {
      batchRead: async (paths: string[]) => {
        ...
        if (!misses.every((p) => extracted.has(p))) {
          ...
          if (looksLikeErrorPayload(full) || looksLikePartialPayload(full)) return full;
          extracted = extractBatchReadMap(full, paths);
          if (!paths.every((p) => extracted.has(p))) return full;
          for (const p of paths) {
            const body = extracted.get(p)!;
            if (allowStoreString(body)) {
              const rk = readKey(p);
              if (rk) this.store.set(rk, body);
            }
          }
          return full;
        }
        ...
        for (const p of misses) fullMap.set(p, extracted.get(p)!);
      ...
      batchSearch: async (items) => {
        ...
        if (!allowStorePayload(out)) return out;
