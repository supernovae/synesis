...
export function extractBatchReadMap(payload: unknown, paths: string[]): Map<string, string> {
  const m = new Map<string, string>();
  if (payload === null || payload === undefined) return m;
  if (typeof payload === "string") {
    if (paths.length === 1) m.set(paths[0], payload);
    return m;
  }
  if (typeof payload !== "object") return m;
  ...
  if (Array.isArray(pathsArr) && Array.isArray(contentsArr) && pathsArr.length === contentsArr.length) {
    for (let i = 0; i < pathsArr.length; i++) {
      ...
      if (typeof p === "string" && typeof c === "string") m.set(p, c);
    }
  }
  ...
  for (const p of paths) {
    const v = o[p];
    if (typeof v === "string") m.set(p, v);
  }

  return m;
}

export function assembleBatchReadPayload(paths: string[], contentsByPath: Map<string, string>): Record<string, unknown> {
  return {
    paths,
...
export function stablePayloadString(payload: unknown): string {
...
export function looksLikeErrorPayload(payload: unknown): boolean {
  ...
  if (typeof payload === "string") {
    ...
    if (s.startsWith("error:") || s.includes("traceback")) return true;
  }
  return false;
}

export function looksLikePartialPayload(payload: unknown): boolean {
  if (payload === null || payload === undefined) return true;
  if (typeof payload === "object" && !Array.isArray(payload)) {
    const o = payload as Record<string, unknown>;
    if (o.partial === true) return true;
    if (o.incomplete === true) return true;
    if (o.streaming === true) return true;
    if (o.status === "in_progress" || o.status === "partial") return true;
  }
  return false;
}
