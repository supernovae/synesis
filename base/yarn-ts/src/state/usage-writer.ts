  ...
  private async upsertSession(session: SessionRecord): Promise<void> {
    ...
    await this.pool.query(
      ...
      ON CONFLICT (session_key) DO UPDATE SET
        ...
        provider = CASE WHEN EXCLUDED.provider != '' AND EXCLUDED.provider != 'synesis' THEN EXCLUDED.provider ELSE yarn_sessions.provider END,
        model = CASE WHEN EXCLUDED.model != '' AND EXCLUDED.model != 'synesis' THEN EXCLUDED.model ELSE yarn_sessions.model END,
      ...
      [
        ...
        session.lastProvider || "synesis",
        session.lastModel || "synesis",
