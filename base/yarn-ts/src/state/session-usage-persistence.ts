...
export interface PersistenceStateChannelPreparation {
  ...
  chatStateSummary: Record<string, unknown> | undefined;
  fileStateSummary: Record<string, unknown> | undefined;
...
function readPersistedChatStateSnapshot(
  metadata: Record<string, unknown>,
...
function readPersistedFileStateSnapshot(metadata: Record<string, unknown>): FileStateSnapshot | null {
...
function summarizeChatSnapshotForPersistence(
...
function summarizeFileStateForPersistence(snapshot: FileStateSnapshot): Record<string, unknown> {
  return {
...
export function preparePersistenceStateChannels(
  ...
  const chatStateSummary = summarizeChatSnapshotForPersistence(persistedChatSnapshot);
  const fileStateSummary = persistedFileSnapshot
    ? summarizeFileStateForPersistence(persistedFileSnapshot)
    : undefined;
