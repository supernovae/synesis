...
const SKILL_KEYWORDS: Array<{ pattern: RegExp; family: string }> = [
...
const LANGUAGE_KEYWORDS: Array<{ pattern: RegExp; language: string }> = [
...
function detectLanguage(text: string): string | null {
  ...
  for (const pack of registry.getAllPacks()) {
    ...
    if (langRe.test(text)) return pack.language;
  }

  return null;
}

function detectSkillFamily(text: string): string {
 ...
 */
export function detectCompositionIntent(
  userText: string,
  workingPhase?: string,
  ...
  if (
    /\b(?:fix|debug|error\b(?!\s+handling)|bug|why\s+(?:is|does|doesn't)|explain|what\s+is)\b/i.test(
      userText,
    )
  ) {
    return null;
  ...
  let confidence = 0.5;
  if (workingPhase === "implement") confidence += 0.15;
  if (COMPOSITION_VERBS.test(userText)) confidence += 0.1;
