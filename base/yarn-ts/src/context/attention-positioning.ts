...
export interface ContextBlock {
  ...
  placement: "begin" | "end" | "any";
}

export interface PositionedMessages {
  messages: Array<{ role: string; content: unknown }>;
  beginBlockCount: number;
  endBlockCount: number;
}

export interface AttentionPositioningStats {
  positionedCount: number;
  beginBlocksPlaced: number;
  endBlocksPlaced: number;
}

function isSystemBlock(content: unknown): boolean {
  const str = typeof content === "string" ? content : "";
  return (
    str.includes("<WORKING_FRAME>") ||
...
function classifyBlock(content: string): ContextBlock {
...
export class AttentionPositioningService {
  private stats: AttentionPositioningStats = {
    positionedCount: 0,
    beginBlocksPlaced: 0,
    endBlocksPlaced: 0
  };
   ...
   */
  position(messages: Array<{ role: string; content: unknown }>): PositionedMessages {
    ...
    for (const b of beginBlocks) result.push({ role: "system", content: b.content });
    for (const b of anyBlocks) result.push({ role: "system", content: b.content });

    for (const m of conversationMessages) result.push(m);

    for (const b of endBlocks) result.push({ role: "system", content: b.content });
  ...
  getStats(): AttentionPositioningStats {
    return { ...this.stats };
  }
}
