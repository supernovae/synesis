...
function inferDesiredEndState(ctx: GapAnalysisContext): string {
  if (ctx.workingFrameGoal) {
    return ctx.workingFrameGoal.slice(0, GOAL_TRUNCATE);
  }

  const text = ctx.userText.trim();
  if (!text) return "Complete the requested task successfully";

  const firstLine = text.split("\n").find((l) => l.trim().length > 0) ?? text;
  return firstLine.slice(0, GOAL_TRUNCATE);
}

function derivePreconditions(gaps: GapClassification): string[] {
  const preconditions: string[] = [];

  for (const gap of gaps.unknown) {
    preconditions.push(
      `Resolve unknown: ${gap.description}`,
    );
  }

  for (const gap of gaps.knowBetter) {
    preconditions.push(
      `Strengthen evidence: ${gap.description}`,
    );
  }
...
function deriveCheckpoints(
  ...
  if (gaps.unknown.length > 0) {
    checkpoints.push(
      `Identify and resolve ${gaps.unknown.length} unknown evidence gap(s)`,
    );
  }

  if (gaps.knowBetter.length > 0) {
    checkpoints.push(
      `Strengthen ${gaps.knowBetter.length} partial evidence item(s) to high confidence`,
    );
  }
...
function actionsForKnowBetterGap(
  gap: { domain: string; description: string },
  ctx: GapAnalysisContext,
): ExplorationAction[] {
  const actions: ExplorationAction[] = [];
...
function buildVerificationToolActions(ctx: GapAnalysisContext): ExplorationAction[] {
  const actions: ExplorationAction[] = [];
...
function generateFallbacks(gaps: GapClassification): string[] {
  const fallbacks: string[] = [];

  if (gaps.unknown.length > 0) {
    fallbacks.push(
      "If exploration does not resolve unknowns, escalate to a higher-capability model tier",
    );
  }

  if (gaps.knowBetter.length > 0) {
    fallbacks.push(
      "If evidence cannot be strengthened sufficiently, proceed with explicit uncertainty caveats",
    );
  }
