  ...
  toSystemBlock(profile: AdapterPackProfile): string {
    ...
    if (profile.client.includes("cursor")) {
      ...
      lines.push(
        ...
        "In Claude Code plan mode, ~/.claude/plans/** is a valid harness-managed write path outside the project root. If the client provides a plan file path, write or update that exact file, then call ExitPlanMode after the plan is ready; do not start implementation while plan mode remains active.",
