    ...
    steps: [
      ...
      },
      {
        ...
        messages: [
          ...
          assistantCall(`${opts.id}-task-2`, "TaskCreate", { title: "Implement core library with domain models and indexing logic" }),
          toolResult(`${opts.id}-task-2`, "task created"),
          assistantCall(`${opts.id}-task-3`, "TaskCreate", { title: "Implement CLI binary with clap subcommands" }),
          toolResult(`${opts.id}-task-3`, "task created"),
          assistantCall(`${opts.id}-task-4`, "TaskCreate", { title: "Add unit and integration tests" }),
          toolResult(`${opts.id}-task-4`, "task created"),
          assistantCall(`${opts.id}-task-5`, "TaskCreate", { title: "Build and verify the workspace compiles" }),
          toolResult(`${opts.id}-task-5`, "task created"),
        ],
        governorOptions: {
          ...baseOptions,
          clientPlanModeRequested: opts.clientPlanModeRequestedAfterApproval,
