export interface JsonCompactionResult {
  compacted: string;
...
function isJsonArray(raw: string): unknown[] | null {
...
function isHomogeneous(items: unknown[]): boolean {
  if (items.length < 3) return false;
  const first = itemFingerprint(items[0]);
  let matchCount = 0;
  for (let i = 1; i < Math.min(items.length, 10); i++) {
    if (itemFingerprint(items[i]) === first) matchCount++;
  }
  return matchCount / Math.min(items.length - 1, 9) >= 0.7;
}

function hasSignalField(item: unknown): boolean {
  if (typeof item !== "object" || item === null) return false;
  const keys = Object.keys(item as Record<string, unknown>);
  return keys.some((k) => SIGNAL_KEYS.includes(k.toLowerCase()));
}

function isAnomalyItem(item: unknown): boolean {
  const str = JSON.stringify(item).toLowerCase();
  return ANOMALY_VALUES.some((v) => str.includes(v));
}

function elbowBudget(n: number): number {
  if (n <= 5) return n;
  if (n <= 20) return Math.ceil(n * 0.5);
  if (n <= 100) return Math.min(15, Math.ceil(n * 0.3));
  return Math.min(20, Math.ceil(Math.sqrt(n) * 2));
}

function selectItems(items: unknown[], budget: number): { selected: unknown[]; indices: number[] } {
  ...
  for (let i = 0; i < items.length && indexSet.size < budget; i++) {
    if (isAnomalyItem(items[i])) indexSet.add(i);
  }
...
export function compactJsonArray(raw: string, opts: CompactionOptions = {}): JsonCompactionResult | null {
  const items = isJsonArray(raw);
  if (!items) return null;

  if (!isHomogeneous(items)) return null;
  if (!hasSignalField(items[0])) {
    const avgItemLen = raw.length / items.length;
    if (avgItemLen > 2000) return null;
  }
