...
export function registeredFamilies(): Array<Exclude<ReducerFamily, "generic">> {
  return Object.keys(REDUCERS) as Array<Exclude<ReducerFamily, "generic">>;
}
...
export class ReducerRegistry {
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    if (family === "generic" || this.options.disabledFamilies.has(family)) return null;
    const reducer = REDUCERS[family as keyof typeof REDUCERS];
