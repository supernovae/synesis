...
export class GitLogReducer implements Reducer {
  readonly family = "git-log" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    if (subjects.length > 0) {
      ...
      if (subjects.length > limit) parts.push(`  ... ${subjects.length - limit} more`);
    }
