...
export class ViteReducer implements Reducer {
  readonly family = "vite" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    for (const line of lines) {
      ...
      if (/error during build|^error:/i.test(t) || /\[vite\].*error/i.test(t)) {
        ...
        /^(dist\/|build\/|\.\/dist\/).+\s+[\d.]+\s*(?:kB|KiB|MB|MiB|bytes?)\b/i.test(t) ||
        /\s[\d.]+\s*kB\s*│/i.test(t)
      ) {
