...
type SeverityKey = "critical" | "high" | "moderate" | "low" | "info";

function parseNpmAuditJson(raw: string): { total: number; bySev: Record<SeverityKey, number>; packages: string[]; fixes: string[] } | null {
  try {
    const j = JSON.parse(raw.trim()) as {
      metadata?: { vulnerabilities?: Partial<Record<SeverityKey | "total", number>> };
      vulnerabilities?: Record<string, { severity?: string; name?: string }>;
    };
    const meta = j.metadata?.vulnerabilities;
    const bySev: Record<SeverityKey, number> = { critical: 0, high: 0, moderate: 0, low: 0, info: 0 };
    let total = 0;
    if (meta) {
      for (const k of ["critical", "high", "moderate", "low", "info"] as const) {
        const n = meta[k];
        if (typeof n === "number") {
          bySev[k] = n;
          total += n;
      ...
      if (total === 0 && typeof meta.total === "number") total = meta.total;
    }
    ...
    const fixes: string[] = [];
    if (raw.includes("audit fix")) fixes.push("npm audit fix");
    if (raw.includes("force")) fixes.push("npm audit fix --force (review)");
...
function parseNpmAuditText(raw: string): { total: number; bySev: Record<SeverityKey, number>; packages: string[]; fixes: string[] } | null {
  const bySev: Record<SeverityKey, number> = { critical: 0, high: 0, moderate: 0, low: 0, info: 0 };
  const packages: string[] = [];
  const fixes: string[] = [];
  ...
  if (paren) {
    ...
    for (const m of inner.matchAll(/(\d+)\s+(critical|high|moderate|low|info)/gi)) {
      ...
      const sev = m[2]!.toLowerCase() as SeverityKey;
      if (sev in bySev) bySev[sev] += n;
    }
  }
  const found = raw.match(/found\s+(\d+)\s+vulnerabilit/i);
  if (found) total = Math.max(total, parseInt(found[1]!, 10));
  ...
  for (const m of pkgBlocks) {
    if (!packages.some((p) => p.startsWith(m[1]!))) packages.push(`${m[1]} ${m[2]}`);
  }

  if (raw.includes("npm audit fix")) fixes.push("npm audit fix");
  if (raw.includes("yarn audit")) fixes.push("yarn upgrade (or apply resolutions)");
  if (raw.includes("Run `npm audit`")) fixes.push("npm audit for details");
  ...
  if (total === 0) total = Object.values(bySev).reduce((a, b) => a + b, 0);
  if (total === 0 && packages.length > 0) total = packages.length;

  return { total, bySev, packages, fixes };
}

export class NpmAuditReducer implements Reducer {
  readonly family = "npm-audit" as const;

  reduce(input: ReducerInput): ReducerOutput | null {
    const raw = input.raw;
    let data = parseNpmAuditJson(raw);
    if (!data) data = parseNpmAuditText(raw);
    if (!data) return null;

    const { total, bySev, packages, fixes } = data;
    const isAudit =
      /vulnerabilit|npm audit|yarn audit|audit report/i.test(raw) ||
      (raw.trim().startsWith("{") && (raw.includes('"vulnerabilities"') || raw.includes('"metadata"')));
    if (!isAudit && total === 0) return null;
    ...
    if (packages.length > 0) {
      ...
      if (packages.length > limit) parts.push(`  ... ${packages.length - limit} more`);
    }
    if (fixes.length > 0) {
