...
export class JavaBuildReducer implements Reducer {
  readonly family = "java-build" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    if (errors.length === 0 && warnings.length === 0 && !buildResult && downloadCount === 0) return null;
    const limit = input.context.profile === "ultra" ? 6 : 12;
    const parts: string[] = [`<TOOL_REDUCED family="java-build" downloads="${downloadCount}">`];
