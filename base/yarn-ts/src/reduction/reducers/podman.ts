...
function truncateRepeating(lines: string[], maxUnique: number): string[] {
  ...
  for (const line of lines) {
    ...
    else if (c === 3) out.push(`  ... (${lines.filter((l) => l === line).length} similar lines omitted)`);
  }
  return out.slice(0, maxUnique);
}

export class PodmanReducer implements Reducer {
  readonly family = "podman" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    if (trimmedStart) {
      try {
        ...
        const first = objs[0] as Record<string, unknown> | undefined;
        if (!first || typeof first !== "object") return null;
        const image =
          (first.Config as { Image?: string } | undefined)?.Image ??
          (first.Image as string | undefined) ??
          (first.ImageName as string | undefined);
        const state = (first.State as { Status?: string; Running?: boolean } | undefined)?.Status;
        const ports =
          first.NetworkSettings && typeof first.NetworkSettings === "object"
            ? JSON.stringify((first.NetworkSettings as { Ports?: unknown }).Ports ?? {})
            : "";
        const mounts = Array.isArray(first.Mounts)
          ? (first.Mounts as { Destination?: string; Source?: string }[])
              .slice(0, 6)
              .map((m) => `${m.Source ?? "?"} -> ${m.Destination ?? "?"}`)
          : [];
        if (!image && !state && mounts.length === 0 && !ports) return null;
        const parts: string[] = [`<TOOL_REDUCED family="podman" containers="${objs.length}">`, "inspect:"];
        if (image) parts.push(`  image: ${image}`);
        if (state) parts.push(`  state: ${state}`);
        if (ports && ports !== "{}") parts.push(`  ports: ${ports.slice(0, 400)}${ports.length > 400 ? "…" : ""}`);
    ...
    if (/^CONTAINER ID\s+/.test(first) || /^CONTAINER ID\t/.test(first)) {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (!row || row.startsWith("CONTAINER")) continue;
      ...
      if (rows.length > limit) parts.push(`  ... ${rows.length - limit} more`);
      parts.push("</TOOL_REDUCED>");
