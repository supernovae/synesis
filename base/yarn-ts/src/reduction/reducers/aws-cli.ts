...
function walkAws(val: unknown, acc: AwsAcc): void {
  if (val === null || val === undefined) return;
  if (typeof val === "string") {
    if (/^i-[0-9a-f]{8,17}$/i.test(val)) acc.instanceIds.push(val);
    if (val.startsWith("arn:aws:")) acc.arns.push(val);
    return;
  }
  if (Array.isArray(val)) {
    for (const v of val) walkAws(v, acc);
    return;
  }
  if (typeof val === "object") {
    const o = val as Record<string, unknown>;
    if (typeof o.InstanceId === "string") acc.instanceIds.push(o.InstanceId);
    if (typeof o.Arn === "string") acc.arns.push(o.Arn);
    const st = o.State;
    if (st && typeof st === "object" && typeof (st as { Name?: string }).Name === "string") {
      acc.states.push((st as { Name: string }).Name);
    }
    const tags = o.Tags;
    if (Array.isArray(tags)) {
      for (const t of tags) {
        if (t && typeof t === "object") {
          ...
          if (tag.Key === "Name" && typeof tag.Value === "string") acc.names.push(tag.Value);
        }
      }
    }
    for (const k of Object.keys(o)) walkAws(o[k], acc);
  }
}
...
function parseAwsTableLines(lines: string[]): { rows: string[]; headers: string[] } {
  const rows: string[] = [];
  ...
  for (const line of lines) {
    ...
    if (
      /\bi-[0-9a-f]{8,17}\b/i.test(joined) ||
...
export class AwsCliReducer implements Reducer {
  readonly family = "aws-cli" as const;

  reduce(input: ReducerInput): ReducerOutput | null {
    const raw = input.raw;
    const lines = raw.split("\n");
    const acc: AwsAcc = { instanceIds: [], arns: [], names: [], states: [] };
      ...
      if (resourceCount === 0 && parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
        ...
        if (Array.isArray(o.Reservations)) {
          ...
          for (const r of o.Reservations) {
            if (r && typeof r === "object" && Array.isArray((r as { Instances?: unknown[] }).Instances)) {
              n += (r as { Instances: unknown[] }).Instances.length;
            }
          }
          resourceCount = n;
        }
    ...
    const parts: string[] = [`<TOOL_REDUCED family="aws-cli" resources="${resourceCount}">`];
