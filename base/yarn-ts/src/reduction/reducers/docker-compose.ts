...
export class DockerComposeReducer implements Reducer {
  readonly family = "docker-compose" as const;

  reduce(input: ReducerInput): ReducerOutput | null {
    const raw = input.raw;
    const lines = raw.split("\n");
    const services = new Map<string, { status?: string; errors: string[] }>();

    const serviceFromLogLine = (line: string): string | null => {
    ...
    for (const line of lines) {
      ...
      if (/^Container\s+/.test(t) && /\bRunning\b|\bExited\b|\bCreated\b/i.test(t)) {
        ...
        if (!services.has(svc)) services.set(svc, { errors: [] });
      ...
      if (/^[a-zA-Z0-9_.-]+\s+.+"(Up|Exit|Restarting|Created|Dead|Paused)/i.test(t)) {
        ...
        const name = cols[0]!;
        if (!services.has(name)) services.set(name, { errors: [] });
        if (/Up/i.test(t)) services.get(name)!.status = "up";
        else if (/Exit|Exited/i.test(t)) services.get(name)!.status = "exited";
        continue;
      }
      ...
      if (svcName && /\b(ERROR|ERR!|FATAL|WARN|WARNING|Exception)\b/i.test(line)) {
        if (!services.has(svcName)) services.set(svcName, { errors: [] });
    ...
    if (services.size === 0 && errLines.length === 0) {
      if (!/compose|Creating|Started|container/i.test(raw)) return null;
    }

    const limit = input.context.profile === "ultra" ? 6 : 12;
    const parts: string[] = [`<TOOL_REDUCED family="docker-compose" services="${Math.max(services.size, errLines.length > 0 ? 1 : 0)}">`];
    ...
    for (const [name, info] of services) {
      ...
      if (info.errors.length > 0) bits.push(`errors=${info.errors.length}`);
      parts.push(`  ${bits.join(" ")}`);
      info.errors.slice(0, 3).forEach((e) => parts.push(`    ${e}`));
      i += 1;
    }
    if (services.size > limit) parts.push(`  ... ${services.size - limit} more services`);
    ...
    if (parts.length <= 2 && errorCount === 0) return null;
