...
export class GradleReducer implements Reducer {
  readonly family = "gradle" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    if (!looksGradle) return null;

    let buildOk: boolean | null = null;
    if (/BUILD SUCCESSFUL/.test(raw)) buildOk = true;
    if (/BUILD FAILED/.test(raw) || /\bFAILURE:\s*Build failed/i.test(raw)) buildOk = false;
    ...
    for (const line of lines) {
      ...
      if (/> Task\s*:/.test(t)) {
        taskLineCount++;
        if (taskLines.length < 40) taskLines.push(t.slice(0, 200));
