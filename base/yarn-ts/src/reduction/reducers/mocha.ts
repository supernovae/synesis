...
export class MochaReducer implements Reducer {
  readonly family = "mocha" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    if (!passFooter && !failFooter && !pendFooter) {
      ...
      for (const line of lines) {
        const t = line.trim();
        if (/^[✓✔]/.test(t)) passTicks += 1;
        if (/^[✗×]/.test(t)) failTicks += 1;
      }
    ...
    if (failureLines.length > 0) {
      ...
      if (failureLines.length > limit) parts.push(`  ... ${failureLines.length - limit} more`);
    }
