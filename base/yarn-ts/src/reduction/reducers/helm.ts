...
export class HelmReducer implements Reducer {
  readonly family = "helm" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    if (!releaseName && errors.length === 0 && resources.length === 0) return null;
    const parts: string[] = [`<TOOL_REDUCED family="helm">`];
