...
export class CmakeReducer implements Reducer {
  readonly family = "cmake" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    for (const line of lines) {
      ...
      if (/^--\s+Found\s/i.test(t)) {
        found++;
        if (foundSamples.length < 24) foundSamples.push(t.slice(0, 220));
      } else if (/^--\s+Could NOT find\s/i.test(t)) {
        missing++;
        if (missingSamples.length < 24) missingSamples.push(t.slice(0, 220));
    ...
    if (/-- Configuring done/i.test(raw)) parts.push("configuring: done");
    if (/-- Generating done/i.test(raw)) parts.push("generating: done");
    if (/-- Build files have been written/i.test(raw)) parts.push("build files written");
    if (foundSamples.length > 0) {
      parts.push("found (sample):");
      foundSamples.slice(0, Math.min(8, limit)).forEach((x, i) => parts.push(`  ${i + 1}. ${x}`));
      if (foundSamples.length > 8) parts.push(`  ... ${found - Math.min(8, limit)} more found lines`);
    }
    ...
    if (cmakeErrors.length > 0) {
      ...
      if (cmakeErrors.length > limit) parts.push(`  ... ${cmakeErrors.length - limit} more`);
    }
