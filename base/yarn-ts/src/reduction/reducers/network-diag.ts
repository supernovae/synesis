...
export class NetworkDiagReducer implements Reducer {
  readonly family = "network-diag" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    for (const line of lines) {
      ...
      if (/^PING\s/.test(trimmed) || /^ping\s/i.test(trimmed)) isPing = true;
      if (/^traceroute\s/i.test(trimmed) || /^tracert\s/i.test(trimmed)) isTrace = true;
      if (/^;\s*(<<>>|DiG)\s/i.test(trimmed) || /^;.*QUERY/.test(trimmed)) isDig = true;
    ...
    if (!pingStats && findings.length === 0) return null;
    const limit = input.context.profile === "ultra" ? 8 : 16;
    const tool = isPing ? "ping" : isTrace ? "traceroute" : isDig ? "dig" : "network";
    const parts: string[] = [`<TOOL_REDUCED family="network-diag" tool="${tool}">`];
    if (pingStats.trim()) parts.push(pingStats.trim());
