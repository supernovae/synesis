...
export class AptPkgReducer implements Reducer {
  readonly family = "apt-pkg" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    let dnfCount = 0;
    if (dnfInstall) dnfCount += parseInt(dnfInstall[1]!, 10);
    if (dnfUpgrade) dnfCount += parseInt(dnfUpgrade[1]!, 10);
    ...
    for (const line of lines) {
      ...
      if (inInstalling && /^\S+\s+(x86_64|aarch64|arm64|noarch)\s+/.test(t)) dnfInstallingLines++;
      if (inInstalling && /^Transaction Summary/i.test(t)) inInstalling = false;
    }
    ...
    if (brewInstalled) brewCount = Math.max(brewCount, parseInt(brewInstalled[1]!, 10));
    if (brewCask) brewCount = Math.max(brewCount, parseInt(brewCask[1]!, 10));

    const looksApt =
      /^Get:\d+\s|^Unpacking\s|^Setting up\s|^Processing triggers for/i.test(raw) ||
      /^Reading package lists/i.test(raw);
    const looksDnf =
      /Transaction Summary|Total download size:|^Installing:\s*$/im.test(raw) && /(dnf|yum)\s/i.test(raw);
    ...
    const parts: string[] = [`<TOOL_REDUCED family="apt-pkg" installed="${installedN}" errors="${errN}">`];
    if (fetched) parts.push(`download: ${fetched}`);
