...
export class PnpmReducer implements Reducer {
  readonly family = "pnpm" as const;

  reduce(input: ReducerInput): ReducerOutput | null {
    const raw = input.raw;
    const lines = raw.split("\n");
    const warnings: string[] = [];
    let packagesAdded: number | null = null;
    let progressLine = "";

    const pkgPlus = raw.match(/Packages:\s*\+(\d+)/i);
    if (pkgPlus) packagesAdded = parseInt(pkgPlus[1]!, 10);
    ...
    for (const line of lines) {
      ...
      if (/^Progress:/i.test(t)) progressLine = t.slice(0, 200);
      if (/^WARN\s/i.test(t) && (/peer|deprecated|unmet/i.test(t) || /engines/i.test(t))) {
        const clip = t.slice(0, 220);
        if (!warnings.includes(clip)) warnings.push(clip);
      }
    ...
    const depsBlock =
      /^dependencies:$/m.test(raw) ||
      /^devDependencies:$/m.test(raw) ||
      /^optionalDependencies:$/m.test(raw);

    const looksPnpm =
      /^pnpm\s+(i|install|add|update)/im.test(raw) ||
    ...
    const parts: string[] = [`<TOOL_REDUCED family="pnpm" packages="${pkgN}" warnings="${warnN}">`];
