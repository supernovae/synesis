...
function parseTrailingPercent(line: string): number | null {
  const percents = [...line.matchAll(/(\d+(?:\.\d+)?)\s*%/g)];
  if (percents.length === 0) return null;
  return Number(percents[percents.length - 1]![1]);
}

export class CoverageReducer implements Reducer {
  readonly family = "coverage" as const;

  reduce(input: ReducerInput): ReducerOutput | null {
    const raw = input.raw;
    const lines = raw.split("\n");
    let totalPct: number | null = null;
    const fileRows: { file: string; pct: number }[] = [];
    ...
    if (fileRows.length === 0 && pipeStmts >= 0) {
      for (let i = pipeStmts + 1; i < lines.length; i++) {
        ...
        if (!file || /^(file|all files|total)$/i.test(file)) continue;
        ...
        if (/^All files$/i.test(file) && m) totalPct = Number(m[1]);
      }
    }

    if (totalPct === null) {
      const totalLine = lines.find((l) => /^TOTAL\b/i.test(l.trim()));
      if (totalLine) totalPct = parseTrailingPercent(totalLine);
    }
    ...
    if (totalPct === null) {
      ...
      if (lf && lh) {
        const a = Number(lf[1]);
        const b = Number(lh[1]);
        if (a > 0) totalPct = Math.round((b / a) * 10000) / 100;
      }
    }
    ...
    if (totalPct === null && fileRows.length === 0 && !/^SF:/m.test(raw)) return null;

    const displayTotal =
      totalPct ?? (fileRows.length > 0 ? Math.round((fileRows.reduce((s, f) => s + f.pct, 0) / fileRows.length) * 100) / 100 : null);
    if (displayTotal === null && below.length === 0 && !/^SF:/m.test(raw)) return null;
    ...
    if (displayTotal !== null) parts.push(`  summary: ${displayTotal}% total`);
    below.slice(0, threshold).forEach((f) => parts.push(`  below 50%: ${f.file} (${f.pct}%)`));
    if (below.length > threshold) parts.push(`  ... ${below.length - threshold} more files below threshold`);
