...
export class MakeReducer implements Reducer {
  readonly family = "make" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    if (errors.length === 0 && warnings.length === 0 && targetCount === 0) return null;
    const limit = input.context.profile === "ultra" ? 6 : 12;
    const parts: string[] = [`<TOOL_REDUCED family="make" targets="${targetCount}">`];
    if (errors.length > 0) {
    ...
    if (errors.length === 0 && warnings.length === 0) parts.push("build completed successfully");
