...
function parseUnifiedDiff(raw: string): { paths: string[]; hunksByFile: string[][]; added: number; removed: number } | null {
  ...
  for (let i = 0; i < lines.length; i++) {
    if (gitRe.test(lines[i]!)) boundaries.push(i);
  }

  if (boundaries.length > 0) {
    const paths: string[] = [];
    const hunksByFile: string[][] = [];
...
export class GitDiffReducer implements Reducer {
  readonly family = "git-diff" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    if (paths.length === 0 && added === 0 && removed === 0) return null;
    ...
    for (let fi = 0; fi < n; fi++) {
      ...
      if (hunks.length > hLimit) parts.push(`    ... ${hunks.length - hLimit} more hunks`);
    }
    if (paths.length > limit) parts.push(`  ... ${paths.length - limit} more files`);
