...
export class SwiftBuildReducer implements Reducer {
  readonly family = "swift-build" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    if (!looksSwift) return null;

    let buildOk: boolean | null = null;
    if (/Build complete!/i.test(raw) && !/\berror:\s/.test(raw)) buildOk = true;
    if (/\berror:\s/.test(raw) || /The following build commands failed:/i.test(raw)) buildOk = false;
    ...
    for (const line of lines) {
      ...
      if (/^error:\s/i.test(t) || (/\berror:\s/.test(t) && /\.swift:\d+:\d+:/.test(t))) {
        ...
        if (targets.length < 30) targets.push(t.slice(0, 200));
      }
    }
    ...
    if (buildOk === true) parts.push("build: complete");
    if (buildOk === false) parts.push("build: failed (errors present)");
    if (targets.length > 0) {
