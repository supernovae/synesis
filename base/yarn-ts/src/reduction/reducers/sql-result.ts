...
export class SqlResultReducer implements Reducer {
  readonly family = "sql-result" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    for (const line of lines) {
      ...
      if (/^\+[-+]+\+$/.test(trimmed) || /^[-]{3,}$/.test(trimmed)) continue;
      if (/^\|.*\|$/.test(trimmed)) {
        if (!headerLine) {
          headerLine = trimmed;
          ...
          rowCount++;
          if (sampleRows.length < 5) sampleRows.push(trimmed);
        }
      } else if (/^\(\d+ rows?\)/.test(trimmed) || /^(\d+) rows? (in set|affected|selected)/.test(trimmed)) {
        const m = trimmed.match(/(\d+)/);
        if (m) rowCount = Math.max(rowCount, Number(m[1]));
    ...
    if (rowCount < 5 && !affectedRows) return null;
    const parts: string[] = [`<TOOL_REDUCED family="sql-result" rows="${rowCount}" cols="${colCount}">`];
    if (headerLine) parts.push(`columns: ${headerLine}`);
    if (sampleRows.length > 0) {
      parts.push("sample:");
      sampleRows.forEach((r) => parts.push(`  ${r}`));
      if (rowCount > sampleRows.length) parts.push(`  ... ${rowCount - sampleRows.length} more rows`);
    }
    if (affectedRows) parts.push(affectedRows);
