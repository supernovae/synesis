...
export class PythonUnittestReducer implements Reducer {
  readonly family = "python-unittest" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    if (failedSummary) {
      ...
      if (failedSummary[2]) errors = parseInt(failedSummary[2]!, 10);
    } else {
    ...
    for (const line of lines) {
      ...
      if (capture && (t.startsWith("Traceback") || t.startsWith("File ") || /^AssertionError/.test(t))) {
        ...
        if (failBlocks.length > 24) capture = false;
      }
    }

    const looksUnittest =
      /Ran\s+\d+\s+tests?\s+in/i.test(raw) ||
      /^FAIL:\s/m.test(raw) ||
      /^ERROR:\s/m.test(raw);

    if (!looksUnittest) return null;
    ...
    if (failBlocks.length > 0) {
      ...
      if (failBlocks.length > limit * 3) parts.push(`  ... ${failBlocks.length - limit * 3} more`);
    }
