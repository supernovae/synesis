...
export class NpmInstallReducer implements Reducer {
  readonly family = "npm-install" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    for (const line of lines) {
      ...
      if (trimmed.startsWith("npm warn") || trimmed.startsWith("npm WARN")) {
        ...
        if (m) addedCount = Number(m[1]);
        const rm = trimmed.match(/removed (\d+)/);
        if (rm) removedCount = Number(rm[1]);
    ...
    if (errors.length === 0 && warnings.length === 0 && addedCount === 0) return null;
    const parts: string[] = [`<TOOL_REDUCED family="npm-install">`];
    if (addedCount > 0) parts.push(`added ${addedCount} packages${removedCount > 0 ? `, removed ${removedCount}` : ""}`);
    if (auditVulns) parts.push(`audit: ${auditVulns}`);
    if (errors.length > 0) {
