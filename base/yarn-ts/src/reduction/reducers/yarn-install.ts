...
export class YarnInstallReducer implements Reducer {
  readonly family = "yarn-install" as const;
  ...
  reduce(input: ReducerInput): ReducerOutput | null {
    ...
    if (resolutionSeen) parts.push("resolution: completed");
    if (fetchSeen) parts.push("fetch: completed");
    if (doneIn) parts.push(`timing: ${doneIn}`);
