...
function walkAz(val: unknown, acc: { names: string[]; types: string[]; locations: string[]; states: string[] }): void {
  if (val === null || val === undefined) return;
  if (Array.isArray(val)) {
    for (const v of val) walkAz(v, acc);
    return;
  }
  if (typeof val === "object") {
    const o = val as Record<string, unknown>;
    if (typeof o.name === "string") acc.names.push(o.name);
    if (typeof o.type === "string") acc.types.push(o.type);
    if (typeof o.location === "string") acc.locations.push(o.location);
    const prov = o.provisioningState;
    if (typeof prov === "string") acc.states.push(prov);
    const ps = o.powerState;
    if (typeof ps === "string") acc.states.push(ps);
    const id = o.id;
    if (typeof id === "string" && id.includes("/resourceGroups/")) acc.names.push(id.split("/").pop() ?? id);
    for (const k of Object.keys(o)) walkAz(o[k], acc);
  }
}

function parseAzTable(lines: string[]): { header: string; rows: string[] } {
  let header = "";
  const rows: string[] = [];
...
export class AzCliReducer implements Reducer {
  readonly family = "az-cli" as const;

  reduce(input: ReducerInput): ReducerOutput | null {
    const raw = input.raw;
    const acc: { names: string[]; types: string[]; locations: string[]; states: string[] } = {
      names: [],
      types: [],
      locations: [],
      states: []
    ...
    const parts: string[] = [`<TOOL_REDUCED family="az-cli" resources="${resourceCount}">`];
