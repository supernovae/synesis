...
class TestAuthorityBoostOrdering:
    ...
    _AUTHORITY_BOOST: ClassVar[dict[str, float]] = {
        "canonical": 1.25,
        "vetted": 1.10,
        "community": 1.00,
        "external": 0.90,
...
class TestRetrievalResultProvenance:
    ...
    def test_community_has_url(self):
        ...
        assert urlparse(chunk.source_url).hostname == "github.com"
...
class TestSearchResultTrustClassification:
    ...
    def test_unmapped_engine_stays_untrusted(self, monkeypatch):
        monkeypatch.setattr("app.web_search.settings.engine_authority_map", {})
    ...
    def test_mapped_engine_becomes_trusted(self, monkeypatch):
        ...
        results = [
            SearchResult(title="Wiki Page", url="https://wiki.internal/page", snippet="test", engine="internal-wiki")
        ]
        classified = classify_results_by_trust(results)
