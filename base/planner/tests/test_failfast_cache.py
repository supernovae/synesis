...
def _fresh_cache(max_size: int = 10, ttl_seconds: float = 3600) -> FailFastCache:
    """Create a cache with an isolated temp file so tests don't leak state."""
    tmp = Path(tempfile.mkdtemp()) / "test_cache.json"
    return FailFastCache(max_size=max_size, ttl_seconds=ttl_seconds, persist_path=tmp)
...
class TestFailFastCache:
    ...
    def test_ttl_expiration(self):
        cache = _fresh_cache(ttl_seconds=0.01)
    ...
    def test_stats(self):
        cache = _fresh_cache(max_size=100, ttl_seconds=7200)
        cache.put("t1", "py", "success", "c")
