...
class QueryNormalizer:
    ...
    def _is_suspicious(self, token: str) -> bool:
        t = token.lower()
        t_stripped = t.rstrip(".,;:!?")
        if len(t_stripped) < self._min_token_len:
            return False
        if t_stripped in self._lexicon or t_stripped in _COMMON_ENGLISH:
            return False
        if self._is_morphological_variant(t_stripped, self._lexicon):
            return False
