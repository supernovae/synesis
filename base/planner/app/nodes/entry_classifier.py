...
def entry_classifier_node(state: dict[str, Any]) -> dict[str, Any]:
    ...
    if not bypass_supervisor:
        if plan_session:
            ...
            escalation_reason = "high_difficulty"
        else:
    ...
    out["brevity_score"] = analysis.get("brevity_score", 0)
    ...
    taxonomy_metadata = resolve_taxonomy_metadata(
        ...
        query_text=out.get("task_description", ""),
    )
    out["taxonomy_metadata"] = taxonomy_metadata

    # Unified difficulty-driven routing: same thresholds for code and non-code.
    # Replaces the always-plan architecture. Only complex tasks get planned.
    # Research: CIE (arXiv:2505.13448) — continuous signals outperform discrete tiers.
    trivial_threshold = float(rt.get("trivial_below", 0.15))

    if difficulty < trivial_threshold and not plan_session:
