"""Strategic Advisor node -- domain classification (models.yaml: router role).

Runs after EntryClassifier, before Supervisor. Infers platform_context
from task_description using the router model (max 15 tokens).
Convention-based: sop_{domain} RAG collections are used when they exist.
"""
...
async def strategic_advisor_node(state: dict[str, Any]) -> dict[str, Any]:
    ...
    complex_threshold = float(rt.get("include_tests_above", 0.7))
    ...
    if difficulty < trivial_threshold or rag_mode == "disabled":
        return {
