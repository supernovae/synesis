"""Executor node -- code generation and explanation (models.yaml: general role).

Receives task + evidence packets from the Router (and optional execution_plan from Planner)
and produces markdown output. Code tasks include fenced code blocks; explanations are
plain markdown. All output streams natively through the OpenAI SDK. Taxonomy steering
(tone, depth, required_elements) and difficulty-based token budgets provide differentiation.
"""
...
- No raw code unless the user explicitly asks for code.
...
_ANTI_BOILERPLATE = """
- Every paragraph must earn its space by addressing the user's specific request. Cut filler.
- Do NOT add generic enterprise scaffolding unless the user explicitly asks for it.
"""
...
def _build_failure_hints(failure_context: list[str]) -> str:
    """Format past failure patterns into guidance for the executor."""
...
def _build_evidence_block(state: dict[str, Any]) -> str:
    """Format evidence packets from the Router into a prompt context block."""
    ...
    for p in packets:
        ...
        for s in sources[:3]:
            ...
            auth = (s.get("metadata", {}) if isinstance(s, dict) else getattr(s, "metadata", {})).get("authority", "")
            badge = f"[R:{auth}]" if auth else ""
            source_refs.append(f"{badge} {uri}")
        refs_str = " | ".join(source_refs) if source_refs else ""
        parts.append(f"[confidence={confidence:.2f}] {refs_str}\n{summary}")
    if not parts:
        return ""
    body = "\n---\n".join(parts)
    return (
...
async def executor_node(state: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        # On execution failure, log only — Router handles retrieval
        if iteration > 0 and execution_result:
            logger.debug(
                "executor_execution_error",
                extra={"iteration": iteration, "result_preview": (execution_result or "")[:200]},
            )
        ...
        if isinstance(execution_plan, dict):
            ...
            constraint_items = [
                a for a in assumptions if isinstance(a, str) and a.lower().startswith("user format constraint")
            ]
            if constraint_items:
                ...
                constraints_block = "\n".join(clines)
                logger.debug("executor_format_constraints_injected", extra={"count": len(constraint_items)})
        ...
        if frame_meta or (frame_format and frame_format != "prose"):
            ...
            for rule in frame_meta:
                frame_parts.append(f"- {rule}")
            if frame_format and frame_format != "prose":
                frame_parts.append(f"- Output format: {frame_format}")
            frame_block = "\n".join(frame_parts)

        task_is_trivial = state.get("task_is_trivial", False)
        trivial_hint = (
            "\n\n**Trivial task** (Supervisor classified). Produce minimal correct code. "
            "Use sensible defaults. Single file unless include_tests. Include run commands. No questions."
            if task_is_trivial
            else ""
        )
        is_code = state.get("is_code_task", False)
        ...
        if active_vertical == "lifestyle" and effective_size == "hard":
            ...
            logger.debug("executor_vertical_override", extra={"vertical": "lifestyle", "effective_size": "medium"})
        ...
        if not is_code_task:
            ...
            if tone:
                ...
                logger.info("executor_taxonomy_tone", extra={"taxonomy_key": taxonomy_key})
            system_prompt += _ANTI_BOILERPLATE
        else:
        ...
        if vertical_block:
            ...
            logger.debug("executor_vertical_injection", extra={"vertical": active_vertical})
        if taxonomy_depth:
            system_prompt = f"{system_prompt}{taxonomy_depth}"
            logger.debug(
                "executor_taxonomy_depth_injection",
        ...
        if not is_code_task:
            ...
            if discovery:
                system_prompt = f"{system_prompt}\n\n{discovery}"
            logger.info("executor_text_mode", extra={"is_code_task": False})

        logger.debug("executor_effective_size=%s", effective_size)
        ...
        if brevity_score > 0 and difficulty < 0.3:
        ...
        if _SOCIAL_ACK_RE.match(task_desc.strip()):
            token_budget = 256
            logger.debug("executor_social_ack_budget", extra={"task": task_desc[:30]})

        task_size = state.get("task_size", "medium")
        logger.debug(
            "executor_token_budget",
        ...
        llm_to_use = executor_llm.bind(max_completion_tokens=token_budget)

        thinking_param = getattr(settings, "executor_thinking_param", "enable_thinking") or ""
        rt = state.get("routing_thresholds") or {}
        thinking_threshold = float(rt.get("thinking_mode_above", 0.7))
        if (
            difficulty >= thinking_threshold
        ...
        if stop_reason:
            ...
            logger.info("executor_stop_reason", extra={"stop_reason": stop_reason})
        ...
        logger.info(
            "executor_completed code_len=%d patch_ops=%d",
            len(generated_code),
            len(patch_ops_list),
        ...
        return {
            "files_touched": [],  # §7.2: Executor must always emit files_touched
            "current_node": node_name,
            "next_node": "respond",
            "error": str(e),
            "node_traces": [trace],
