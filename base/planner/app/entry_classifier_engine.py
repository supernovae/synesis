...
Intent classification uses BM25-inspired scoring (term frequency saturation,
document length normalization, IDF weighting) instead of first-match.
See docs/WORKFLOW.md ref #10 for design rationale.
...
# BM25 parameters for intent classification scoring.
# Ref: Robertson & Zaragoza, "The Probabilistic Relevance Framework: BM25
# and Beyond" (2009). Stanford NLP IR Book Ch. 11.4.
_BM25_K1 = 1.5  # TF saturation (slightly above default 1.2 for multi-word keyword phrases)
_BM25_B = 0.75  # document length normalization
_BM25_AVG_DL = 50  # assumed average prompt length in words
...
_DASH_BULLET_RE = re.compile(r"^\s*[-*]\s+\S", re.MULTILINE)
_ACTION_VERB_RE = re.compile(
    r"^\s*[-*]?\s*(?:state|propose|explain|describe|list|give|show|provide|outline|define|identify|compare|evaluate|recommend|summarize|discuss|design|analyze|assess|justify|prioritize)\b",
    re.MULTILINE | re.IGNORECASE,
)


def _count_explicit_deliverables(text: str) -> int:
    """Count explicit deliverables the user is asking for.

    Detects numbered lists (1. / 2.), dash-bullet asks (- Explain X),
    and action-verb sentence starters. Returns the max of any pattern
    to avoid double-counting overlapping formats.
    """
    numbered = len(_NUMBERED_ITEM_RE.findall(text))
    dashed = len(_DASH_BULLET_RE.findall(text))
    action_verbs = len(_ACTION_VERB_RE.findall(text))
    return max(numbered, dashed, action_verbs)
...
class ScoringEngine:
    ...
    def __init__(self, config_path: str | Path | None = None):
        ...
        bw = self._config.get("brevity_weights") or {}
        ...
        self._brevity_patterns = _build_patterns(bw)
    ...
    def analyze(self, text: str) -> dict[str, Any]:
        ...
        # 5b. Brevity weights: negative adjustments for simplicity/brevity cues.
        # Reduces complexity_score (floors at 0) to push difficulty toward 0.
        # Research: CIE (arXiv:2505.13448) — continuous signals > discrete for
        # verbosity control. UCL (arXiv:2601.00880) — over-specification
        # degrades output; reducing complexity for simple tasks improves quality.
        brevity_score = 0
        for cat, (weight, pattern) in self._brevity_patterns.items():
            matches = pattern.findall(t_lower)
            if matches:
                abs_weight = abs(weight)
                brevity_score += abs_weight
                complexity_score = max(0, complexity_score + weight)
                hits.append(f"brevity:{cat}({weight})")
                score_breakdown[f"brevity_{cat}"] = weight

        # 6. Deliverable counter: detect explicit numbered/bulleted asks.
        # Research: IFScale (arXiv:2507.11538) — LLMs degrade on multi-instruction
        # following; structural enforcement of instruction count outperforms reliance
        # on the LLM alone. AdaCtrl (arXiv:2505.18822) — difficulty-aware budgets.
        explicit_deliverables = _count_explicit_deliverables(t)
        if explicit_deliverables > 3:
            bonus = (explicit_deliverables - 3) * 2
            complexity_score += bonus
            hits.append(f"deliverable_bonus(+{bonus},count={explicit_deliverables})")
            score_breakdown["deliverable_bonus"] = bonus
        ...
        # 8. Intent class — BM25-scored: score all intents, highest wins.
        # Code intents preferred within 10% of best score (tie-breaking).
        ...
        doc_len = max(1, len(t_lower.split()))
        n_intents = max(1, sum(1 for v in intent_classes.values() if isinstance(v, dict)))

        # IDF: keywords appearing in fewer intent classes are more discriminating
        kw_intent_count: dict[str, int] = {}
        for ic_data in intent_classes.values():
            if not isinstance(ic_data, dict):
                continue
            seen_in_class: set[str] = set()
            for kw in ic_data.get("keywords", []):
                kw_key = str(kw).lower()
                if kw_key not in seen_in_class:
                    kw_intent_count[kw_key] = kw_intent_count.get(kw_key, 0) + 1
                    seen_in_class.add(kw_key)

        len_norm = 1.0 - _BM25_B + _BM25_B * (doc_len / _BM25_AVG_DL)

        intent_scores: dict[str, float] = {}
        intent_matched_kws: dict[str, list[str]] = {}
        for ic_name, ic_data in intent_classes.items():
            ...
            for kw in ic_data.get("keywords", []):
                kw_str = str(kw)
                tf = len(re.findall(rf"\b{re.escape(kw_str)}\b", t_lower))
                if tf > 0:
                    n_containing = kw_intent_count.get(kw_str.lower(), 1)
                    idf = math.log((n_intents - n_containing + 0.5) / (n_containing + 0.5) + 1.0)
                    saturated_tf = (tf * (_BM25_K1 + 1.0)) / (tf + _BM25_K1 * len_norm)
                    ic_score += idf * saturated_tf
                    matched.append(kw_str)
            if ic_score > 0:
                intent_scores[ic_name] = ic_score
                intent_matched_kws[ic_name] = matched

        if intent_scores:
            best_score = max(intent_scores.values())
            threshold = best_score * 0.9
            candidates = [ic for ic, sc in intent_scores.items() if sc >= threshold]
            code_candidates = [ic for ic in candidates if ic in code_intents]
            intent_class = code_candidates[0] if code_candidates else candidates[0]
            hits.append(
                f"intent:{intent_class}"
                f"(bm25={intent_scores[intent_class]:.2f}"
                f",kws={intent_matched_kws.get(intent_class, [])})"
            )
        ...
        # 9. is_code_task — semantic classifier replaces regex code_rescue.
        # Embedding-based cosine similarity handles "mention vs intent" naturally:
        # "Write a Python function" → code; "architecture supporting Python" → knowledge.
        # Research: Semantic Router (Aurelio Labs), Routesplain (arXiv:2511.09373),
        # VecStat/NormStat (ICLR 2026) — embedding methods outperform keyword classifiers
        # for ambiguous and out-of-distribution prompts.
        from .semantic_intent import classify_code_intent

        ic_data = intent_classes.get(intent_class) if isinstance(intent_classes.get(intent_class), dict) else {}
        semantic_threshold = float((self._config.get("semantic_classifier") or {}).get("confidence_threshold", 0.05))
        if ic_data.get("inherently_document"):
            is_code_task = False
            hits.append("is_code_task:false(inherent)")
        else:
            is_code, margin = classify_code_intent(t, threshold=semantic_threshold)
            is_code_task = is_code
            hits.append(f"is_code_task:{is_code_task}(semantic:{margin:.2f})")
