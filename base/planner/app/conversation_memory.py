...
class ConversationMemory:
    ...
    def __init__(
        ...
        self._last_context: dict[str, tuple[str, list[str]]] = {}  # user_id -> (deliverable_type, active_domain_refs)
    ...
    def get_last_context(self, user_id: str) -> tuple[str, list[str]] | None:
        """Return (deliverable_type, active_domain_refs) for context pivot detection. None if never set."""
        with self._lock:
            return self._last_context.get(user_id)

    def set_last_context(self, user_id: str, deliverable_type: str, active_domain_refs: list[str]) -> None:
        """Store deliverable_type and active_domain_refs after a turn for next-turn pivot detection."""
        with self._lock:
            self._last_context[user_id] = (deliverable_type, list(active_domain_refs or []))
            self._last_active[user_id] = time.time()
