...
# ---------------------------------------------------------------------------
# Router decision schema (V3: minimal JSON-lite output from Router LLM)
# ---------------------------------------------------------------------------


class RouterDecision(BaseModel):
    """Minimal routing decision from the Router LLM (Qwen3-8B).
...
class ExecutorOut(BaseModel):
    ...
    # Worker can request more info instead of guessing
...
# ---------------------------------------------------------------------------
# StyleContract and FinalAnswerAudit — compiler pipeline schemas
# ---------------------------------------------------------------------------


class RawExtractionCandidate(BaseModel):
    """A single span extracted by GLiNER2 in Stage 1."""
...
class FirstPassFrame(BaseModel):
    """Stage 1 output: raw candidates grouped by field, before normalization."""

    main_question_candidates: list[RawExtractionCandidate] = Field(default_factory=list)
    requirements: list[RawExtractionCandidate] = Field(default_factory=list)
    constraints: list[RawExtractionCandidate] = Field(default_factory=list)
    negative_constraints: list[RawExtractionCandidate] = Field(default_factory=list)
    deliverables: list[RawExtractionCandidate] = Field(default_factory=list)
    formats: list[RawExtractionCandidate] = Field(default_factory=list)
    technologies: list[RawExtractionCandidate] = Field(default_factory=list)
    domain_tags: list[RawExtractionCandidate] = Field(default_factory=list)
    timeline_signals: list[RawExtractionCandidate] = Field(default_factory=list)
    quality_instructions: list[RawExtractionCandidate] = Field(default_factory=list)
    decision_signals: list[RawExtractionCandidate] = Field(default_factory=list)
    escalation_signals: list[RawExtractionCandidate] = Field(default_factory=list)
...
class StyleContract(BaseModel):
    ...
    by difficulty) enforce length; no separate word targets, paragraph
    caps, or conciseness booleans.
    """
...
# ---------------------------------------------------------------------------
# Anti-oscillation framework schemas
# ---------------------------------------------------------------------------


class DecisionEntry(BaseModel):
    """A single architecture/approach decision from the planner.

    Stored in ``decision_ledger`` (append-only). Structured writer
    consumes these instead of raw planner prose. Critic validates draft
    against ledger entries.
    """

    decision_id: str
    category: Literal["architecture", "tooling", "style", "scope", "approach"]
    chosen: str
...
class CritiqueItem(BaseModel):
    """A single critique finding with resolution tracking.

    Stored in ``critique_register`` (merge-by-key, forward-only status).
    Status transitions: open -> resolved -> settled.
    """

    item_id: str
    category: str
    description: str
    status: Literal["open", "resolved", "settled"]
    evidence_ref: str = ""
    resolved_by: str = ""
    reopen_count: int = 0
...
class OverrideRequest(BaseModel):
    """Explicit request to change a frozen decision or style rule.

    Narrow scope (``this_section``) with substantive reason auto-approves.
    Broad scope (``all_sections``, ``permanent``) requires critic confirmation.
    """

    target_decision_id: str
    override_reason: str
    override_scope: Literal["this_section", "all_sections", "permanent"]
    requested_by: str
    approved: bool = False
