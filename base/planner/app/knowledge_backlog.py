...
def update_gap_status(
    ...
    try:
        ...
        with_retry(_get_resilient(), lambda c: c.upsert(collection_name=STATUS_COLLECTION, data=[entity]))
...
def delete_gap(chunk_id: str) -> bool:
    ...
    try:
        ...
        with_retry(
            _get_resilient(),
            lambda c: (
                c.delete(collection_name=COLLECTION, filter=f'chunk_id == "{chunk_id}"'),
                c.delete(collection_name=STATUS_COLLECTION, filter=f'chunk_id == "{chunk_id}"'),
            ),
        )
