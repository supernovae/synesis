...
_ACTION_MATRIX: dict[_MatrixKey, PolicyDecision] = {
    ...
    (Severity.LOW, ConfidenceBand.LOW): PolicyDecision(
        action=PolicyAction.ALLOW, severity=Severity.LOW,
        scope=Scope.REQUEST, notify_channels=[], reason="Low confidence, log only",
    ),
    (Severity.LOW, ConfidenceBand.MEDIUM): PolicyDecision(
        action=PolicyAction.WARN, severity=Severity.LOW,
        scope=Scope.REQUEST, notify_channels=["dashboard"], reason="Low severity, warn",
    ),
    (Severity.LOW, ConfidenceBand.HIGH): PolicyDecision(
        action=PolicyAction.WARN, severity=Severity.LOW,
        scope=Scope.REQUEST, notify_channels=["dashboard"], reason="Low severity, high confidence",
    ),
    # MEDIUM severity
    (Severity.MEDIUM, ConfidenceBand.LOW): PolicyDecision(
        action=PolicyAction.WARN, severity=Severity.MEDIUM,
        scope=Scope.REQUEST, notify_channels=["dashboard"], reason="Medium severity, low confidence",
    ),
    (Severity.MEDIUM, ConfidenceBand.MEDIUM): PolicyDecision(
        action=PolicyAction.SANITIZE, severity=Severity.MEDIUM,
        scope=Scope.SESSION, notify_channels=["dashboard"], reason="Medium severity and confidence; sanitize",
    ),
    (Severity.MEDIUM, ConfidenceBand.HIGH): PolicyDecision(
        action=PolicyAction.SANITIZE, severity=Severity.MEDIUM,
        scope=Scope.SESSION, notify_channels=["dashboard"], reason="Medium severity, high confidence; sanitize + monitor",
    ),
    # HIGH severity
    (Severity.HIGH, ConfidenceBand.LOW): PolicyDecision(
        action=PolicyAction.SANITIZE, severity=Severity.HIGH,
        scope=Scope.SESSION, notify_channels=["dashboard"],
        reason="High severity, low confidence; sanitize and watch",
    ),
    (Severity.HIGH, ConfidenceBand.MEDIUM): PolicyDecision(
        action=PolicyAction.BLOCK, severity=Severity.HIGH,
        scope=Scope.SESSION, notify_channels=["dashboard", "webhook"],
        reason="High severity, medium confidence; block request",
    ),
    (Severity.HIGH, ConfidenceBand.HIGH): PolicyDecision(
        action=PolicyAction.RESTRICT_TOOLS, severity=Severity.HIGH,
        scope=Scope.SESSION, ttl_seconds=1800, notify_channels=["dashboard", "webhook"],
        reason="High severity+confidence; block + restrict tools (30 min)",
    ),
    # CRITICAL severity
    (Severity.CRITICAL, ConfidenceBand.LOW): PolicyDecision(
        action=PolicyAction.BLOCK, severity=Severity.CRITICAL,
        scope=Scope.SESSION, notify_channels=["dashboard", "webhook"],
        reason="Critical severity, low confidence; block pending review",
    ),
    (Severity.CRITICAL, ConfidenceBand.MEDIUM): PolicyDecision(
        action=PolicyAction.FREEZE_TOKEN, severity=Severity.CRITICAL,
        scope=Scope.TOKEN, ttl_seconds=3600, notify_channels=["dashboard", "webhook", "pager"],
        reason="Critical severity, medium confidence; freeze token (60 min)",
    ),
    (Severity.CRITICAL, ConfidenceBand.HIGH): PolicyDecision(
        action=PolicyAction.FREEZE_TOKEN, severity=Severity.CRITICAL,
        scope=Scope.TOKEN, ttl_seconds=3600, requires_approval=False,
...
def lookup(
    ...
    if template is None:
        return PolicyDecision(
            action=PolicyAction.WARN, severity=severity,
            scope=Scope.REQUEST, reason=f"No matrix entry for {key}",
        )
